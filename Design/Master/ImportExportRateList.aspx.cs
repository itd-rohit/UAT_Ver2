...
public partial class Design_Master_ImportExportRateList : System.Web.UI.Page
{
    ...
    private void LoadBillCategory()
    {
        DataTable dt;
        dt = StockReports.GetDataTable("SELECT ID,NAME FROM `billingcategory_master` WHERE IsActive=1");
        ddlBillCategory.DataSource = dt;
        ddlBillCategory.DataTextField = "NAME";
        ddlBillCategory.DataValueField = "ID";
        ddlBillCategory.DataBind();
        ddlBillCategory.Items.Insert(0, new ListItem("All", "0"));
    }
    ...
    public static string DownloadRateList(string PanelId, string PanelName, string BillCategoryID)
    {

        StringBuilder sb = new StringBuilder();
        sb.Append(" SELECT im.ItemID,im.type_id,im.testcode,sc.name, bcm.`Name` BillCategory,REPLACE(REPLACE(im.typeName,',',''),'´','')  ItemName,IFNULL(r.Rate,0)Rate ");
        sb.Append(" FROM f_itemmaster im ");
        sb.Append(" INNER JOIN f_subcategorymaster sc ON sc.SubCategoryID=im.SubCategoryID ");
        sb.Append(" INNER JOIN `billingcategory_master` bcm ON im.billcategoryid=bcm.`ID` ");
        sb.Append(" LEFT OUTER JOIN f_ratelist r ON r.itemid=im.ItemID AND r.panel_id='" + PanelId + "' ");
