...
public partial class Design_Websitedata_FeedBack : System.Web.UI.Page
{
    ...
    {
        lblerrmsg.Text = "";
        //grd.DataSource = StockReports.GetDataTable("SELECT * FROM feedback order by id  ");
        //grd.DataBind();
    }
    [WebMethod(EnableSession = true)]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public static string BindFeedBack(string MobileNo)
    {
        try
        {
            DataTable dt = StockReports.GetDataTable("SELECT ID,FBNAME,DATE_FORMAT(FBDOV,'%d-%b-%Y')FBDOV,FBADDRESS,FBMBNO,FBEMAILID,FBVISTITIME,FBKNOWABT,FBWHYCCL,FBREGPROCESS,FBSTAFF,FBBLOODTIME,FBTECH,FBREPORT,FBREPSERVICE,FBRATE FROM feedback  WHERE FBMBNO LIKE '%" + MobileNo + "' order by id ");
