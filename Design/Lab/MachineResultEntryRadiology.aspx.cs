...
public partial class PatientResultEntry : System.Web.UI.Page
{
   ...
   " ORDER BY fa.isDefault DESC,em.Name ",
    ...
    private void BindMachine(MySqlConnection con)
    {
        try
        {
            ddlMachine.DataSource = MySqlHelper.ExecuteDataset(con, CommandType.Text, "SELECT MachineID FROM `machost_atulya`.`mac_machinemaster` where centreid=@CentreID  ORDER BY MachineID",
    ...
    private void BindMacMaster(MySqlConnection con)
    {
        try
        {
            ddlMacMaster.DataSource = MySqlHelper.ExecuteDataset(con, CommandType.Text, "SELECT  groupid ,MachineID FROM `machost_atulya`.`mac_machinemaster` where centreid=@CentreID  ORDER BY MachineID",
               new MySqlParameter("@CentreID", UserInfo.Centre)).Tables[0];
            ddlMacMaster.DataTextField = "MachineID";
            ddlMacMaster.DataValueField = "groupid";
    ...
    public static string bindAccessCentre()
    {
	 ...
	 [WebMethod(EnableSession = true)]
    public static string checkreport(string Test_ID)
    {
        try
        {
            DataTable dtdetails = StockReports.GetDataTable("SELECT isReportOpen,Date_Format(ReportOpenDateTime,'%d-%b-%y %H:%i')ReportOpenDateTime,Date_Format(ReportOpenDateTime,'%d-%b-%y %I:%i%p')ReportOpenDateTime1,ReportOpenBy,0 DateAllowed,0 SameUser FROM Patient_LabInvestigation_OPD WHERE Test_Id = '" + Test_ID + "'");
            if (Util.GetString(dtdetails.Rows[0]["ReportOpenBy"]).Split('(')[0] == UserInfo.LoginName)
            {
                dtdetails.Rows[0]["SameUser"] = "1";
    ...
    [WebMethod(EnableSession = true)]
    public static string updateIsopen(string Test_ID)
    {
        try
        {
    ...
    public static string GetPanelMaster(string centreid)
    {
        ...
        try
        {
            ...
            if (centreid == "ALL")
            {
                ...
                q1 += " INNER JOIN f_panel_master pm ON cp.PanelId=pm.panel_id and cp.CentreId in(select  cm.CentreID from centre_master cm where ( cm.CentreID in (select CentreAccess from centre_access where CentreID =@CentreID ) or cm.CentreID = @CentreID) and cm.isActive=1) and pm.isactive=1 order by company_name";
            }
            else
            {
                q1 = "SELECT  pm.`Panel_ID`,CONCAT (IF(IFNULL(pm.panel_code,'')='',pm.`Panel_ID`,pm.panel_code), ' ~ ',pm.company_name) company_name  FROM Centre_Panel cp   ";
                q1 += " INNER JOIN f_panel_master pm ON cp.PanelId=pm.panel_id and cp.CentreId=Centre_ID and pm.isactive=1 order by company_name";
    ...
    public static string PatientSearch(string SearchType, string SearchValue, string FromDate, string ToDate, string CentreID, string SmpleColl, string Department, string MachineID, string MacGroupid, string ZSM, string TimeFrm, string TimeTo, string isUrgent, string InvestigationId, string PanelId, string SampleStatusText, string chremarks, string chcomments, string BookingUser, string ColourCode, string addon, string ResultType, string camp, string TATOption)
    {
        ...
        try
        {
            ...
            sbQuery.Append("  DATE_FORMAT(pli.date, '%d-%b-%y %H:%i') RegDate,DATE_FORMAT(pli.SampleReceiveDate, '%d-%b-%y %H:%i') DATE,pli.`LedgerTransactionNo`,pli.SubCategoryID,'' SampleLocation,pli.CombinationSample,");
            ...
            sbQuery.Append(" ,pli.AgeInDays AGE_in_Days,lt.ClinicalHistory  ");
            ...
            if (BookingUser != "0")
            {
                sbQuery.Append(" and lt.Creator_UserID=@Creator_UserID");
            }
            ...
            else
            {
                ...
                if (SmpleColl == " and pli.isSampleCollected<>'N' ")   //All Patient
                {
                    //sbQuery.Append("   ( if(pli.isSampleCollected='S',pli.issamplecollected='S' and pli.CentreID=@CentreID   , pli.TestCentreID=@CentreID)))  ");
					sbQuery.Append("   ( if(pli.isSampleCollected='S',pli.issamplecollected='S' and pli.CentreID in (SELECT CentreAccess FROM centre_access WHERE CentreID=@CentreID)   , pli.TestCentreID in (SELECT CentreAccess FROM centre_access WHERE CentreID=@CentreID) )))  ");
                }
                else
                {
                    //sbQuery.Append("   (  pli.TestCentreID=@CentreID ");
					sbQuery.Append("   (  pli.TestCentreID in (SELECT CentreAccess FROM centre_access WHERE CentreID=@CentreID) ");
            ...
            if (((SearchValue.Trim() == string.Empty) && (SearchType.Trim() != "lt.Patient_ID")) || ((SearchValue.Trim() == string.Empty) && (SearchType.Trim() != "pli.BarcodeNo")))
            {
 ...
 if(UserInfo.RoleID==15)
           ...
           // sbQuery.Append(" AND pli.IsReporting='1' GROUP BY  lt.`LedgerTransactionNo`,pli.SubCategoryID");
            ...
            if (SampleStatusText == "Tested" && ColourCode == string.Empty)
            {
                sbQuery.Append(" order by pli.isurgent asc,TATDelay desc, pli.SampleReceiveDate asc ");
            }
            else
            {
                sbQuery.Append(" order by pli.isurgent Desc,TATDelay desc, pli.SampleReceiveDate asc ");
            }
    ...
    //System.IO.File.WriteAllText(@"F:\Lims 6.0\live_Code\Mdrc\ErrorLog\mac.txt",sbQuery.ToString());
            using (MySqlDataAdapter da = new MySqlDataAdapter(String.Format(sbQuery.ToString(), string.Join(",", InvID), string.Join(",", ObservationType_ID)), con))
            {
                ...
                da.SelectCommand.Parameters.AddWithValue("@Creator_UserID", BookingUser);
    ...
    public static string LabObservationSearch(string LabNo, string TestID, float AgeInDays, string RangeType, string Gender, string MachineID, string macId)
    {
        ...
        try
        {
            ...
            sbObs.Append(" ( SELECT LabNo,LabObservation_ID,Reading MacReading,Reading1,Reading2,Reading3, MachineID,machinename,MachineID1,MachineID2,MachineID3,MacDate,Test_ID FROM  mac_data WHERE LedgerTransactionNo=@LedgerTransactionNo and ( Reading<>'' or Reading1<>'')  ");
            ...
            //if(UserInfo.ID ==1)
              //System.IO.File.WriteAllText (@"F:\Lims 6.0\live_Code\Mdrc\ErrorLog\macdta.txt", sbObs.ToString());
