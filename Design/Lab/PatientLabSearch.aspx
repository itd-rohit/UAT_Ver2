<%@ Page ClientIDMode="Static" Language="C#" MasterPageFile="~/Design/DefaultHome.master" AutoEventWireup="true" CodeFile="PatientLabSearch.aspx.cs" Inherits="Design_Lab_PatientLabSearch" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div id="Pbody_box_inventory">
        ...
        <div class="POuter_Box_Inventory">
            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    <asp:CheckBox ID="chkPanel" runat="server" onClick="$bindPanel();" Text="Panel :" Style="font-weight: 700"   />
                </div>
                <div class="col-md-4">
            ...
            <div class="row">
                ...
                <div class="col-md-5">
                    <asp:DropDownList ID="ddlUser" runat="server" class="ddlUser  chosen-select chosen-container">
                    </asp:DropDownList>
                </div>
            </div>
            <div class="row">
                ...
                <div class="col-md-2">
                    <input id="btnprint" type="button" value="Print" class="resetbutton" onclick="PrintReport()" />
                </div>
            ...
            <div class="row">
                    ...
                    <div class="col-md-1 square badge-DepartmentReceive" style=" height: 20px;width:2%; float: left;border:groove; " onclick="$patientLabSearch('3','');">
                    </div>
            ...
            <div class="POuter_Box_Inventory">
                ...
                <div id="PagerDiv1" style="display: none; background-color: white; width: 100%; padding-left: 7px;scroll">
                </div>
 ...
 <td class="GridViewHeaderStyle">SRF No.</td>
                            ...
                            <%if (Util.GetString(Request.QueryString["covid"]) == "1")
                            {%>
                        <td class="GridViewHeaderStyle" style="width:100px">Covid</td>
    ...
    <script type="text/javascript">
        var isCovid = '<%=Util.GetString(Request.QueryString["covid"])%>';
        function $bindCentre() {
    ...
    <script type="text/javascript">
        ...
        function $patientLabSearch(stype, pageno) {
            ...
            serverCall('PatientLabSearch.aspx/SearchPatient', { searchData: $searchData, stype: stype, PageNo: _$PageNo, PageSize: _$PageSize, InvIdList: InvIdList }, function (response) {
                      ...
                      //  $('#PagerDiv1').html('');
                       ...
                       // $('#spnTestCount').html($testcount);
                        ...
                        for (var i = 0; i < $responseData.length; i++) {
                            ...
                            $myData.push('<td class="GridViewLabItemStyle"><b>'); $myData.push($responseData[i].SRFNo); $myData.push('</b></td>');
                            ...
                            $myData.push('<td class="GridViewLabItemStyle" style="display:none" id="tdreporttype">'); $myData.push($responseData[i].ReportType); $myData.push('</td>');
                            $myData.push('<td class="GridViewLabItemStyle" style="display:none" id="tdRadiologyfilepath">'); $myData.push($responseData[i].Radiologyfilepath); $myData.push('</td>');
                            if (isCovid == "1") {
                                $myData.push('<td class="GridViewLabItemStyle">'); $myData.push('<b><a href="CovidRegistration.aspx?LedgerTransactionID='); $myData.push($responseData[i].LedgerTransactionID); $myData.push('" target="_blank">'); $myData.push($responseData[i].CovidStatus); $myData.push('</a></b></td>')
                            }
                            $myData.push('<td class="GridViewLabItemStyle">');
                            ...
                            $myData.push('<td class="GridViewLabItemStyle">');
                            if ($responseData[i].Approved == "1") {
                                $myData.push('<img src="../../App_Images/print.gif" style="cursor:pointer;" onclick="PrintReport(0)"/>');
                            }
                            $myData.push('</td>');
                            $myData.push('<td class="GridViewLabItemStyle">');
                            if ($responseData[i].Approved == "1" && $responseData[i].ShowPrintPrev>0 ) {
							...
							$myData.push(' onclick="callShowAttachment(\'' + $responseData[i].LedgerTransactionNo+ '\')" id="btnDocumentMaster"/></td>');
                            $myData.push("</tr>");
                            $myData = $myData.join("");
            ...
            $('#tb_ItemList tr').each(function () {
                ...
                if (id != "header") {
                    if ($(this).closest("tr").find('#mmchk').prop('checked') == true) {
                        debugger;
                        if ($(this).closest("tr").find("#tdreporttype").text() == "11") {
                            radiologytestid += id + ",";
                            window.open("RadiologyReport.aspx?TestID=" + id + "&filepath=" + $(this).closest("tr").find("#tdRadiologyfilepath").text());
                        }
                        else
                            testid += id + ",";
                        if (IsPrev != "1") {
        ...
        function viewDetail(barcode) {
            //var BarcodeNo = $(rowID).closest("tr").find('#td_BarcodeNo').text();
            serverCall('PatientLabSearch.aspx/PostData', { ID: barcode }, function (response) {
                $responseData = JSON.parse(response);
        ...
        function PrintSmartReport(LabNo) {
            serverCall('PatientLabSearch.aspx/PostData', { ID: LabNo }, function (response) {
                $responseData = JSON.parse(response);
                window.open("PatientReportPdf.aspx?LabNo=" + $responseData.LabID, null, 'left=150, top=100, height=400, width=850,  resizable= no, scrollbars= yes, toolbar= no,location= no, menubar= no');
            });
        }
        function PrintSmartReportheader(LabNo) {
            serverCall('PatientLabSearch.aspx/PostData', { ID: LabNo }, function (response) {
                $responseData = JSON.parse(response);
                window.open("PatientReportPdf.aspx?PHead=1&LabNo=" + $responseData.LabID, null, 'left=150, top=100, height=400, width=850,  resizable= no, scrollbars= yes, toolbar= no,location= no, menubar= no');
            });
        }
    </script>
