...
public partial class Design_Lab_MoneyReceiptNew1 : System.Web.UI.Page
{
           ...
           if (dtObs.Rows.Count > 7)
           {
            ...
            //    {
                    ...
                    //    }
                        ...
                        //sb.AppendFormat("<td style='text-align:right;'>{0}</td>", Math.Round(Util.GetDecimal(dwc["Amount"]), Util.GetInt(dwc["CurrencyRoundDigit"])), MidpointRounding.AwayFromZero);
    ...
    private string MakeHeader()
    {
        ...
        Header.Append("<table style='width:800px;  border-collapse:collapse;font-family:Times New Roman;font-size:17px;background-color: #C8C8C8;'><br><br><br>");
        Header.Append("<tr>");
        ...
        Header.Append("<td style='width:100px;font-weight:bold;'><i>MR-No </i>: " + Util.GetString(drcurrent["ReceiptNo"]));
        Header.Append("</td>");
      
        Header.Append("<td><i>MR-Date</i>: " + DateTime.Now.ToString("dd-MMM-yyyy hh:mm tt"));
        //Header.AppendFormat("<td>{0}</td>", DateTime.Now.ToString("dd-MMM-yyyy hh:mm tt"));
        Header.Append("</td>");
        Header.Append("</tr>");
        Header.Append("<tr>"); 
     //   Header.Append("<td><br>Visit-Date:", Util.GetString(drcurrent["regdate"]));

        Header.Append("<td style='width:20px;'><i>Visit-Date </i> : " + Util.GetString(drcurrent["regdate"]));
        Header.Append("</td>");
        Header.Append("</tr>");
        // Header.AppendFormat("<td>{0}</td>", Util.GetString(drcurrent["regdate"]));
        //Header.AppendFormat("<td>{0}/{1}</td>", Util.GetString(drcurrent["regdate"]));
       Header.Append("<tr><td><i>Received with thanks from</i> : <b>" + Util.GetString(drcurrent["UserName"]));
       // Header.Append("<tr><td colspan='9'><b>Received with thanks :</b> " + Util.GetString(drcurrent["AmountInWord"]));
       Header.Append("</td>");
      //  Header.Append("<td><i>against ID</i>: " + Util.GetString(drcurrent["patient_id"]));
       ...
       //Header.AppendFormat("<td>{0}</td>", Util.GetString(drcurrent["AmountInWord"]));
        Header.Append("</tr>");
        Header.Append("<tr>");
        ...
        Header.Append("</tr>");
        Header.Append("<tr>");
         Header.Append("<td style='font-weight:bold;'><i>Rs.</i> " + Util.GetFloat(drcurrent["Adjustment"]));
        Header.Append("</td>");
