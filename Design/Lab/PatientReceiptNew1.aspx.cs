...
public partial class Design_Lab_PatientReceiptNew1 : System.Web.UI.Page
{
    ...
    //Header Property
    const float HeaderHeight = 185;//207//178
    ...
    private void SetHeader(PdfPage page)
    {
        ...
        if (drcurrent["HeaderImage"].ToString().Length > 0)
            HeaderImage = true;
        if (drcurrent["isShowHeader"].ToString() == "1")
        {
            if (HeaderImage)
            {
    ...
    private void SetFooter(PdfPage page)
    {
        if (page.Footer != null)
        {
            if (drcurrent["isShowFooter"].ToString() == "1")
            {
                if (FooterImage)
                {
    ...
    {
        PdfImage transparentResizedPdfImage = new PdfImage(10, 300, Server.MapPath(SignImg)) { PreserveAspectRatio = true };
        //transparentResizedPdfImage.AlphaBlending = true;
        return transparentResizedPdfImage;
        //imageLayoutInfo = page1.Layout(transparentResizedPdfImage);
    }
    ...
    private string MakeHeader()
    {
        ...
        Header.Append("<td></td>");
		Header.Append("<td style='font-weight:bold;'>INSTITUTOR</td>");
        Header.AppendFormat("<td>{0}</td>", Util.GetString(drcurrent["OtherLabRefNo"]));
	    Header.Append("</tr>");
    ...
    private void BindData()
    {
        ...
        try
        {
           ...
           //    new MySqlParameter("@LedgerTransactionID", labid)));
            ...
            sb.Append(" cm.isShowHeader,cm.isShowFooter, pm.`Age`,pm.`Gender`,pm.`Mobile`, lt.`PanelName`,IF(lt.`DoctorName`='SELF',lt.`DoctorName`,CONCAT('Dr.', lt.`DoctorName`))Doctorname, ");
