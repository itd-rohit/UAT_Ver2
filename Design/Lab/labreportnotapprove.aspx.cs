...
public partial class Design_Lab_labreportnotapprove : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            if (Util.GetString(Request.QueryString["GetReportBase64"]) != "1")
            {
                ...
                if (UserInfo.RoleID != 177)// open For Adminsitartor Role If DueUserInfo.RoleID != 177 ||
                {
                    try
                    {
                        validation objVal = new validation();
        ...
        ||
                ...
                dtObs.Rows[dtObs.Rows.IndexOf(dr) - 1]["PackageName"].ToString() != dr["PackageName"].ToString()
                 ...
                 ||
                  ...
                  dtObs.Rows[dtObs.Rows.IndexOf(dr) - 1]["ApprovedBy"].ToString() != dr["ApprovedBy"].ToString()
                        ...
                        if (signatureonend == true)
                        {
                            ...
                            if (list.Length > 0)
                            {
                                ...
                                foreach (DataRow st in deptapp.Rows)
                                {
                                    ...
                                    {
                                        string imgname = string.Format("{0}Design/opd/signature/{1}.jpg", serverPath, Util.GetString(st["ApprovedBy"]));
                                        sbimg.AppendFormat("<img style='float:left;padding-right:10px;' src='{0}'", imgname);
                                        sbimg.Append(" />");
                                    }
                                }
                ...
                ((_FirstRow) ? true : (dtObs.Rows[dtObs.Rows.IndexOf(dr) - 1]["Test_ID"].ToString() != dr["Test_ID"].ToString())))
            {
                ...
                sb.AppendFormat("<th  colSpan='{0}' class='InvName'>", dtLabReport.Rows.Count);
            ...
            if (dr["investigation_id"].ToString() != "001")//&& dr["investigation_id"].ToString() != "1187"
            {
                ...
                else if (dr["isOrganism"].ToString() == "0")
                {
                    ...
                    else
                    {
                        for (int j = dtLabReport.Rows.Count - 1; j >= 0; j--)
                        {
                            ...
                            if (dr["isborder"].ToString() == "1")
                            {
                                ...
                                {
                                    newTD = string.Format("<td style='border:0px;text-align:left ! important;' class='{0}' colSpan='{1}'>{2}</td>{3}", drc["Name"], colSpan, getTags_Flag(dr[drc["Name"].ToString()].ToString(), dr["MinValue"].ToString(), dr["MaxValue"].ToString(), dr["AbnormalValue"].ToString(), drc["Name"].ToString().ToLower(), dr, dr["flag"].ToString()), newTD);
                                    colSpan = 1;
                                }
                                else
                                {
                            ...
                            else
                            {
                                if (dr[drc["Name"].ToString()].ToString() != "")
                                {
                                    ...
                                    newTD = string.Format("<td style='text-align:left ! important;'  class='{0}' colSpan='{1}'>{2}</td>{3}", drc["Name"], colSpan, getTags_Flag(dr[drc["Name"].ToString()].ToString(), dr["MinValue"].ToString(), dr["MaxValue"].ToString(), dr["AbnormalValue"].ToString(), drc["Name"].ToString().ToLower(), dr, dr["flag"].ToString()), newTD);
                                    colSpan = 1;
                                }
                                else
                                {
                    ...
                    //if (dtObs.Select(string.Format("Test_ID='{0}'", dr["Test_ID"])).Length == 1)
                    //{
                ...
                )
            {
                if (dr["InterpretationID"].ToString() != "0" && dr["InterpretationType"].ToString() == "InvestigationWise")
                {
                    ...
                    {
                        sb.Append("<tr valign='top'>");
                        sb.Append("<td style='padding:8px;' colSpan='" + dtLabReport.Rows.Count + "'><br/><b>Comment:</b><br/>" + Interpretation + "</td>");
                        sb.Append("</tr>");
                    }
                }
            }
                ...
                )
            {
                ...
                sb.Append("<tr valign='top'>");
                sb.AppendFormat("<td class='Department' style='border-left:0px;border-right:0px;border-bottom:0px;text-align:left;'>****Sample Collected By Patient Itself</td>");
                sb.Append("</tr>");
            }
            ...
            if ((_LastRow) ? true : (dtObs.Rows[dtObs.Rows.IndexOf(dr) + 1]["Test_ID"].ToString() != dr["Test_ID"].ToString()))
            {
                ...
                if (_Attachments.Rows.Count > 0)
                {
                    ...
                    {
                        string _img = string.Format("{0}uploaded document/{1}", serverPath, _Attachments.Select(string.Format("Test_ID='{0}' and AttachedFile like '%.jpg'", dr["Test_ID"]))[0]["fileurl"]);
                        sb.Append("<tr valign='top'>");
        ...
        try
        {
            //micro report
            int isculture = 0;
            using (MySqlCommand cmd = new MySqlCommand(string.Format("SELECT count(1) from patient_labobservation_opd_mic where TestID in({0})", Test_ID), con))
            {
                for (int i = 0; i < Test_ID.Count; i++)
                {
                    cmd.Parameters.AddWithValue(Test_ID[i], Test_ID[i]);
                }
            ...
            int ishisto = 0;
            using (MySqlCommand cmd = new MySqlCommand(string.Format("SELECT count(1) from patient_labobservation_histo where TestID in({0})", Test_ID), con))
            {
                for (int i = 0; i < Test_ID.Count; i++)
                {
                    cmd.Parameters.AddWithValue(Test_ID[i], Test_ID[i]);
            ...
            if (ishisto > 0)
            {
                string url = string.Format("{0}://{1}:{2}/{3}/Design/Lab/labreportnewhisto.aspx{4}", Request.Url.Scheme, Request.Url.Host, Request.Url.Port, Request.Url.AbsolutePath.Split('/')[1], Request.Url.Query);
                Stream stream = ConvertToStream(url);
    ...
    private void mergeDocument()
    {
        ...
        foreach (PdfPage p in tempDocument.Pages)
        {
            ...
            PdfText pageNumberingText = new PdfText(PageWidth - 50, FooterHeight + 80, String.Format("Page {0} of {1}", pageno, tempDocument.Pages.Count), pageNumberingFont);
        ...
        try
        {
            ...
            else
            {
                if ((MinRange != "") && (Util.GetFloat(Value) < Util.GetFloat(MinRange)))
                    ret_value = string.Format("<span style='color:red;'><strong>{0}<strong></span>", ret_value); // ret_value = "<b>"+ ret_value + "</b>";
    ...
    void htmlToPdfConverter_PageCreatingEvent(PdfPageCreatingParams eventParams)
    {
        ...
        //}
        Font sysFontBold = new Font("Arial", 60, FontStyle.Regular, GraphicsUnit.Point);
        PdfText titleRotatedText = new PdfText(50, 550, "Amended Report", sysFontBold) { ForeColor = Color.LightGray, RotationAngle = 45 };
        page1.Layout(titleRotatedText);
        SetHeader(page1);
        SetFooter(page1);
    }
    private void AddContent(string Content, DataRow dr, bool rowin)
    {
        ...
        if (showItdose)
        {
            ...
            PdfText titleRotatedText = new PdfText(2, -20, "ITDOSE INFOSYSTEMS PVT. LTD.", sysFontBold) { ForeColor = Color.Gray, RotationAngle = 90 };
            page1.Layout(titleRotatedText);
        }
    }
    private void SetHeader(PdfPage page)
    {
        ...
        if (drcurrent["ispackage"].ToString() == "1")
        {
            HeaderHeight = Util.GetFloat(drcurrent["ReportHeaderHeight"].ToString()) + 21;
        }
        else
        {
            HeaderHeight = Util.GetFloat(drcurrent["ReportHeaderHeight"].ToString());
    ...
    private void bindReport()
    {
        ...
        sb.Append("  t.LabNo,t.PLOID,t.laboutsrcid,t.FieldExe,t.InterpretationID,t.InterpretationType,   ");
        sb.Append("   substr(t.PName,1,35)PName, t.Age, t.BarcodeNo, t.Gender,t.ReportStatus,t.OtherReferLab,t.Interface_companyName,t.CorporateIDCard,t.PatientIDProofNo,t.LedgerTransactionNo_Interface,   ");
        sb.Append("   t.DateEnrolled, t.Patient_ID, t.Description, t.SampleBySelf, t.LabObservation_ID, t.Test_ID,   ");
        ...
        sb.Append("   t.ApprovedBy, t.ResultEnteredDate,  ");
        ...
        sb.Append("  t.centreid,t.Panel_ID,t.ReferDoctor,t.Company_Name ,t.Company_code,t.Company_add,t.`HLMPatientType`,t.`HLMOPDIPDNo`,t.IsSampleCollectedByPatient,t.PatientType,");
        ...
        sb.Append(" FROM (      ");
        sb.Append(" SELECT   ");
        sb.Append(" pli.LedgerTransactionNo AS LabNo,pli.test_id PLOID,pli.laboutsrcid,IF(lt.HomeVisitBoyID='N/A','N/A',(SELECT  flm.NAME FROM feildboy_master flm WHERE flm.FeildBoyID=lt.HomeVisitBoyID)) FieldExe,pli.InterpretationID,pli.InterpretationType,");
        sb.Append("CASE WHEN lt.Ismask > 0 THEN PMV.Paitent_name  ELSE lt.PName END as PName , lt.Age,pli.BarcodeNo,left(lt.Gender,1)Gender,IF(pli.Approved=0,'Provisional Report','Final Report')ReportStatus,lt.OtherReferLab,lt.Interface_companyName,lt.CorporateIDCard,lt.PatientIDProofNo,lt.LedgerTransactionNo_Interface, ");
        sb.Append(" DATE_FORMAT(pli.date,'%d/%b/%Y %I:%i%p') DateEnrolled,lt.Patient_ID,pl.Description,pli.SampleBySelf,pl.LabObservation_ID,pl.Test_ID, ");
        ...
        sb.Append("  pli.ApprovedBy,        DATE_FORMAT(pli.ResultEnteredDate,'%d/%b/%Y %I:%i%p')ResultEnteredDate, ");
        ...
        sb.Append("  (SELECT isborder FROM labobservation_investigation WHERE investigation_id=pli.investigation_id AND LabObservation_ID=pl.LabObservation_ID) isborder,pl.MachineID  , lt.centreid,lt.Panel_ID,Concat('Dr.',lt.`DoctorName`) ReferDoctor, IF(lt.OtherReferLab='',left(lt.panelname,33),lt.OtherReferLab)  Company_Name ,'' Company_code,'' Company_add,lt.`HLMPatientType`,lt.`HLMOPDIPDNo`,pli.IsSampleCollectedByPatient,IFNULL(lt.PatientType,'')PatientType ");
        sb.Append("  FROM patient_labObservation_opd_notApprove  pl   ");
        sb.Append("   INNER JOIN patient_labInvestigation_opd_notApprove  pli ON pli.Test_ID = pl.Test_ID AND TRIM(pl.value)<>'' AND pli.ReportType='1' and  pli.UnapproveDate = pl.UnapproveDate  and pli.UnapproveDate=@UnapproveDate ");
        ...
        sb.Append("  SELECT t.*, ");
        sb.Append("  '' AS Interpretation,");
        sb.Append(" '3' ReportType,IF(IM.PrintSampleName=0,IF(IFNULL(itm.Inv_ShortName,'')!='',IM.Name,IM.Name),im.name) AS TestName,im.method,");
        sb.Append(" im.FileLimitationName,scm.deptinterpretaion,");
        sb.Append("  otm.Name AS Dept,otm.isVisible AS DeptVisible, ");
        sb.Append(" IM.Print_sequence AS Print_Sequence_Investigation ,");
        sb.Append(" im.PrintSeparate,im.printsamplename, ");
        sb.Append(" IM.Description AS d1,'' Priorty,'' IsParent,im.printHeader   ");
        sb.Append("  AS invHeader    ,");
        sb.Append(" '' SampleTypeID,cma.Centre CentreName,cma.Address,cma.Landline CentreLandline,cma.Mobile CentreMobile,cma.Email CentreEmail,cma.NablLogoPath,cma.CapLogoPath,'' printOrder ,cma.type1,");
        sb.Append("  IFNULL((SELECT isNABL FROM `investiagtion_isnabl` WHERE Investigation_ID=im.Investigation_Id AND centreid=t.CentreID),0)isNABL ");
        ...
        sb.Append(" SELECT  ");
        sb.Append("  pli.LedgerTransactionNo AS LabNo,pli.test_id PLOID,pli.laboutsrcid,IF(lt.HomeVisitBoyID='N/A','N/A',(SELECT  flm.NAME FROM feildboy_master flm WHERE flm.FeildBoyID=lt.HomeVisitBoyID)) FieldExe,pli.InterpretationID,pli.InterpretationType,");
        sb.Append("   lt.PName,lt.age Age,pli.BarcodeNo,left(lt.Gender,1)Gender,IF(pli.Approved=0,'Provisional Report','Final Report')ReportStatus ,lt.OtherReferLab,lt.Interface_companyName,lt.CorporateIDCard,lt.PatientIDProofNo,lt.LedgerTransactionNo_Interface, ");
        sb.Append(" DATE_FORMAT(pli.date,'%d/%b/%Y %I:%i%p') DateEnrolled,lt.Patient_ID Patient_ID,'' Description,pli.SampleBySelf,'' LabObservation_ID,pli.Test_ID, ");
        sb.Append(" DATE_FORMAT(pli.ResultEnteredDate,'%d-%b-%y')AS ResultDate,pli.ResultEnteredBy ,if(pli.ForwardBy=pli.ApprovedBy,'',pli.ForwardBy) ForwardBy ,       ");
        sb.Append(" (SELECT LabInves_Description FROM patient_labobservation_opd_text_notapprove  WHERE PLO_ID = pli.test_id) VALUE,'' `MinValue`,'' `MaxValue`, '' Flag,'' abnormalimage,pli.sampletypename samplename,'' LabObservationName,pli.Investigation_ID,pli.SlideNumber,pli.TestCentreID, ");
        sb.Append("     DATE_FORMAT(pli.SampleCollectionDate,'%d/%b/%Y %I:%i%p')SampleDate, ");
        ...
        sb.Append(" , lt.centreid,lt.Panel_ID,Concat('Dr.',lt.`DoctorName`) ReferDoctor, IF(lt.OtherReferLab='',left(lt.panelname,33),lt.OtherReferLab) Company_Name ,'' Company_code,'' Company_add,lt.`HLMPatientType`,lt.`HLMOPDIPDNo`,pli.IsSampleCollectedByPatient,IFNULL(lt.PatientType,'')PatientType");
        sb.Append(" FROM patient_labInvestigation_opd_notApprove  pli    ");
        sb.Append(" INNER JOIN f_ledgertransaction lt ON lt.LedgerTransactionID=pli.LedgerTransactionID  ");
        sb.Append(" AND pli.Test_ID IN ({0}) and pli.UnapproveDate=@UnapproveDate  AND pli.Result_Flag = 1  AND pli.ReportType=3 AND pli.ishold=0   ");
        sb.Append("  )t    ");
        ...
        //System.IO.File.WriteAllText (@"F:\Shat\q2.txt", sb.ToString());
        ...
        try
        {
            using (MySqlDataAdapter da = new MySqlDataAdapter(String.Format(sb.ToString(), inClause), con))
            {
                ...
                da.SelectCommand.Parameters.AddWithValue("@UnapproveDate", Request.QueryString["unid"]);
                da.Fill(dtObs);
            }
            if (dtObs.Rows.Count == 0)
            {
