...
public partial class Design_Lab_PatientLabSearch : System.Web.UI.Page
{
    ...
    public static string SearchPatient(object searchData, string stype, string PageNo, string PageSize, string InvIdList)
    {
        ...
        {
                string rresult = Util.DateDiffPatientsearch(Util.GetDateTime(labSearch[0].ToDate), Util.GetDateTime(labSearch[0].FromDate));
        ...
        try
        {
            string InvoicePanelID = StockReports.ExecuteScalar("select Invoiceto from f_panel_master where employee_ID=" + Util.GetString(UserInfo.ID) + " ");
            ...
            if ((Util.GetString(labSearch[0].Panel) ==""))
            {
                 if (UserInfo.RoleID == 220)
              {
            ...
            switch (stype)
            {
                ...
                case "6":
                    ...
                    break;
                case "7":
                    sb.Append(" AND  plo.Result_Flag=1  and (plo.Approved is null or plo.Approved=0) and isForward=0 and plo.isHold=0  AND plo.isPartial_Result=0 "); //Tested
                    break;
                case "8":
            ...
            if (InvIdList != "")
                sb.Append(" AND plo.investigation_id in (" + InvIdList + ") ");

            if (labSearch[0].UserID != "All")
            {
                sb.Append(" AND lt.`CreatedByID`=@CreatedByID");
            }
            if (Util.GetString(UserInfo.CentreType) == "PUP")
            {
                sb.Append("  AND lt.CentreID=@PCCCentre and  lt.panel_id=@PCCPanel ");
            }
        //   else if( UserInfo.LoginType == "PCC" || UserInfo.LoginType.ToUpper() == "SUBPCC")
        //   {
        //       sb.Append("  AND lt.CentreID=@CentreID");
        //   }
            else if (labSearch[0].Centre != "ALL")
            {
                sb.Append("  AND lt.CentreID=@CentreID");
            }
            else
            {
                ...
                if (UserInfo.PanelType == "PCC" || UserInfo.PanelType == "SUBPCC")  //Remove Condition after some time
                {
                    sb.Append("  AND ( lt.CentreID in ( select ca.CentreAccess from  centre_access ca where ca.Centreid=@SessionCentreID )  or lt.CentreID=@SessionCentreID or lt.centreId=1) ");
                }
                else if (RoleId != "212")
                {
            ...
            //if (UserInfo.LoginType == "PCC")
            //{
            //    sb.Append(" and fpm.Employee_ID="+ UserInfo.ID +" ");
            //}
            if (UserInfo.PanelType == "PCC")
            {
                sb.Append(" and fpm.InvoiceTo ='" + InvoicePanelID + "'");
            }
            else if (UserInfo.PanelType == "SUBPCC")
            {
                sb.Append(" and fpm.employee_ID=" + UserInfo.ID + "");
            }
           ...
           // System.IO.File.WriteAllText(@"F:\Lims 6.0\live_Code\Mdrc\ErrorLog\lis.txt", sb.ToString());
            using (DataTable dt = MySqlHelper.ExecuteDataset(con, CommandType.Text, sb.ToString(),
                   ...
                   new MySqlParameter("@CreatedByID", labSearch[0].UserID),
                   new MySqlParameter("@PCCCentre", UserInfo.Centre),
                   new MySqlParameter("@PCCPanel", UserInfo.ID)).Tables[0])
            {
              //  if (PageNo == "0")
              //  {
                    if (dt.Rows.Count > 0)
    ...
    private void bindData()
    {
        ...
        try
        {
            ...
            string[] deptTags = String.Join(",", Util.GetString(HttpContext.Current.Session["AccessDepartment"])).Split(',');
