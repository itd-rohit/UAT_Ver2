...
" ORDER BY fa.isDefault DESC,em.Name ");
    ...
    private void BindMachine()
    {
        try
        {
            ddlMachine.DataSource = StockReports.GetDataTable("SELECT MachineID FROM  " + Util.getApp("MachineDB") + ".`mac_machinemaster` ORDER BY MachineID");
    ...
    private void BindDepartment()
    {
        ...
        sb.Append("  SELECT `SubCategoryID`,NAME FROM f_subcategorymaster WHERE active=1 AND `CategoryID`='LSHHI3'   ");
            ...
            sb.Append("  and  SubCategoryID in ('42') ");
    ...
    {
        DataTable dt = StockReports.GetDataTable("select IF(im.TestCode='',typename,CONCAT(im.TestCode,' ~ ',typename)) testname,im.`ItemID` testid from f_itemmaster im where isactive=1 and im.subcategoryid='42' order by testname");
        return Newtonsoft.Json.JsonConvert.SerializeObject(dt);
    }
    ...
    public static string PatientSearch(string SearchType, string SearchValue, string FromDate, string ToDate, string CentreID, string SmpleColl, string Department, string MachineID, string ZSM, string TimeFrm, string TimeTo, string isUrgent, string InvestigationId, string PanelId, string SampleStatusText, string ColourCode, string chremarks, string chcomments, string camp, string TATOption, string isTATDate)
     {
        ...
        sbQuery.Append("  SELECT  '' ReferLab ,pli.Investigation_ID,pli.Approved,DATE_FORMAT(pli.incubationdatetime, '%d-%b-%Y') incubationdate,DATE_FORMAT(pli.incubationdatetime, '%h:%i:%s') incubationtime,pli.reportnumber, ");
        sbQuery.Append("  DATE_FORMAT(pli.SampleReceiveDate, '%d-%b-%y %H:%i') DATE,pli.`LedgerTransactionNo`,'' SampleLocation,pli.CombinationSample,lt.`PName`,CONCAT(lt.`Age`,'/',lt.`Gender`) Age_Gender,lt.`Gender`,lt.Patient_ID,");
        sbQuery.Append(" lt.doctorname AS Doctor,cm.Centre,pli.`BarcodeNo`,pli.SampleTypeName, lt.panelname,  im.Name AS Test,pli.Test_ID Test_ID");
        sbQuery.Append(" ,pli.AgeInDays AGE_in_Days ");
        sbQuery.Append(",  (SELECT COUNT(*) FROM `patient_labinvestigation_opd_remarks` WHERE Test_ID= pli.Test_ID and IsActive='1') RemarkStatus");
        ...
        else
        {
            ...
            sbQuery.Append("  AND pli.Date >='" + (Util.GetDateTime(FromDate).ToString("yyyy-MM-dd") + " " + TimeFrm) + "' ");
            sbQuery.Append(" AND pli.Date <='" + Util.GetDateTime(ToDate).ToString("yyyy-MM-dd") + " " + TimeTo + "' ");
        }
        ...
        if ((SearchType.Trim() == "lt.Patient_ID") || ((SearchValue.Trim() == "") && (SearchType.Trim() == "pli.BarcodeNo")) && ColourCode == "")
        {
            if (SampleStatusText != "Tested")
            {
                if (SampleStatusText == "Pending")
        ...
        if (ColourCode != "")  // 
        {
            ...
            {
                sbQuery.Append("  and pli.CultureStatus='Incubation' and pli.Result_flag=0  and pli.isSampleCollected<>'N'  and pli.isSampleCollected<>'R'    ");
            }
                ...
                if (temp != dr["Test_ID"].ToString())
                {
                    ...
                    if (tempDept != dr["Dept"].ToString())
                    {
                        ...
                        if (micro == "" || micro == "0")
                        {
                            try
                            {
                                micro = Util.GetString(StockReports.ExecuteScalar("SELECT GROUP_CONCAT(DISTINCT mm.id) FROM micro_master mm INNER JOIN  " + Util.getApp("MachineDB") + ".`mac_observation_vitek` mov ON mov.ObservationName=mm.code AND mov.labno='" + dr["barcodeno"].ToString() + "'"));
                            }
                            catch (Exception) { 
 ...
 CASE WHEN IFNULL(plomic.`testid`,'0')='0' THEN 'Pending'  WHEN IFNULL(plomic.Result_flag,'0')='1' && IFNULL(plomic.Approved,'0')='0' && ifnull(plo.isforward,'0')='0'  THEN 'Tested' WHEN IFNULL(plomic.Approved,'0')='1' THEN 'Approved'
 WHEN plo.ishold=1 THEN 'Hold' WHEN IFNULL(plomic.Result_flag,'0')='1' && IFNULL(plomic.Approved,'0')='0' && ifnull(plo.isforward,'0')='1'  THEN 'Forwarded' END mystatus FROM `patient_labinvestigation_opd` plo  LEFT JOIN `patient_labobservation_opd_mic` plomic ON plo.`Test_ID`=plomic.`testid`  and plomic.reporttype='" + reportnumber + "' WHERE plo.`Test_ID`='" + dr["Test_ID"].ToString() + "' limit 1");
                    ...
                    if (dtreportfile.Select("test_id='" + drCopy["Test_ID"].ToString() + "'").Length > 0)
                    {
                        ...
                        {
                           // var url = az.fileShowFromAzure(dwreport["AttachedFile"].ToString(), dwreport["filePath"].ToString(), "uploadedreport");
                           ...
                           // drNew1["Value"] = string.Format("<a href='{0}' target='_blank' >{1}</a>", url, dwreport["AttachedFile"].ToString());
    ...
    public static string Comments_LabObservation(string LabObservation_ID, string Type)
    {
        if (Type == "Value")
        {
            ...
            string str = "SELECT it.Comments_ID Template_ID,it.Comments_Head Temp_Head FROM `investigation_comments` it " +
    ...
    public static string GetComments_labobservation(string CmntID, string type)
    {
        ...
        if (type == "Value")
        {
            ...
            str = "SELECT Comments Template_Desc FROM `investigation_comments` WHERE `Comments_ID`='" + CmntID + "'";
        }
        else
        {
    ...
    public static string SaveLabObservationOpdData(string reportnumber, List<Patient_lab_ObservationOPD_MIC> antibioticdata, List<ResultEntryProperty> data, string ResultStatus, string ApprovedBy, string ApprovalName, string HoldRemarks,  string notapprovalcomment)
    {
        ...
        try
        {
                    ...
                    if (ResultStatus == "Not Approved")
                    {
                        ...
                        MySqlHelper.ExecuteNonQuery(tnx, CommandType.Text, "call Insert_UnApprove_Plo(@Test_ID,@UnapproveDate,@LedgerTransactionNo,@UnapprovebyID,@Unapproveby,@Comments,@ipaddress,@ReportType)",
                           ...
                           new MySqlParameter("@ipaddress", StockReports.getip()),
                            new MySqlParameter("@ReportType", 2)); 

                        MySqlHelper.ExecuteNonQuery(tnx, CommandType.Text, "UPDATE patient_labinvestigation_opd SET NotApprovedCount=NotApprovedCount+1 ,Approved = @Approved, isForward = @isForward, isPrint = @isPrint,reportnumber=@reportnumber WHERE Test_ID=@Test_ID AND isSampleCollected=@isSampleCollected",
            ...
            if (ResultStatus == "Approved")
            {
               // MySqlHelper.ExecuteScalar(tnx, CommandType.Text, "update patient_labobservation_opd_mic set Approved=1,ApprovedBy='" + UserInfo.ID + "',ApprovedByname='" + UserInfo.LoginName + "',ApprovedDateTime=now() WHERE `Testid`=@Test_ID and reporttype=@reporttype", new MySqlParameter("@Test_ID", myplotestid), new MySqlParameter("@reporttype", reportnumber));
		  MySqlHelper.ExecuteScalar(tnx, CommandType.Text, "update patient_labobservation_opd_mic set Approved=1,ApprovedBy='" + ApprovedBy + "',ApprovedByname='" + ApprovalName + "',ApprovedDateTime=now() WHERE `Testid`=@Test_ID and reporttype=@reporttype", new MySqlParameter("@Test_ID", myplotestid), new MySqlParameter("@reporttype", reportnumber));
            }
            ...
            if (ItemData != "")
            {
                for (int i = 0; i < len; i++)
                {
                    ...
                    if (Type == "CR")
                    {
                        ...
                        //sb.Append(" SampleSegratedBy_ID='" + UserInfo.ID + "', ");
                        //sb.Append(" SampleSegratedBy = '" + UserInfo.LoginName + "',SegratedDate=NOW(), ");
                        //sb.Append(" UpdateID='" + Item[i].Split('_')[1].ToString() + "', ");
                        //sb.Append(" UpdateName='" + UserInfo.ID + "', ");
    ...
    public static string BindobsAntibiotic(string obid, string obname, string testid, string Barcodeno, string reportnumber)
    {
        ...
        sb.Append(" LEFT  JOIN  " + Util.getApp("MachineDB") + ".`mac_observation_vitek` mov ON  mm.`code`=mov.`Machine_ParamID` AND mov.`Type`='2' and mov.labno='" + Barcodeno + "' AND mm.`code` IS NOT NULL and mov.ObservationName=(SELECT CODE FROM micro_master WHERE id='" + obid + "' ) ");
        ...
        sb.Append(" (CASE WHEN   Interpretation='S' THEN 'Sensitive' WHEN   Interpretation='I' THEN 'Intermediate' WHEN  Interpretation='R' ");
    ...
    [WebMethod(EnableSession = true)]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public static string BindReportno(string  investicationId)
    {
        try
        {


            string dt = StockReports.ExecuteScalar(" SELECT  MicroReportType FROM investigation_master WHERE `Investigation_Id`=" + investicationId + "  ");
