...
public partial class Design_Reports_LabReport_TATReportPdf : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            try
            {
                ...
                sb.Append(" SELECT PatientName,AgeSex,UHID,IPDOPD as IPDOPD,TestDetails,Department,RegDateTime,SampleCollDateTime,SampleReceiveArea,");
                sb.Append(" SampleReceiveDate,ResultEnteredDate,ResultEnteredName, ApproveDateTime ,ApprovedName , LabNo,BarcodeNo,PanelName, '' RESPONSIBILITY, '' REASONFORDELAY,Deviaion ");
               // sb.Append(" TIMEDIFF(ApproveDateTime,RegDateTime) ActualTAT1   ");
                //sb.Append(" DeliveryDate SelectedDeliveryDate, ");
               
                sb.Append(" FROM  ");
                sb.Append("(SELECT lt.HisWorkOrderID AS UHID ,IF( IF(plo.Approved=0,NOW(),plo.ApprovedDate) > plo.DeliveryDate,'Outside TAT','Within TAT') ActualTAT,IF(lt.`HisIsIPDOPD`=1,'OPD',IF(lt.`HisIsIPDOPD`<>'' AND lt.`HisIsIPDOPD`=0 ,'IPD',''))IPDOPD, plo.BarcodeNo, DATE_FORMAT(plo.ResultEnteredDate,'%d-%b-%Y %h:%i %p')ResultEnteredDate,plo.ResultEnteredName,lt.LedgerTransactionNo LabNo, lt.PName As PatientName, CONCAT(lt.Age,LEFT(lt.Gender,1))AgeSex, ");
                ...
                sb.Append("  DATE_FORMAT( lt.date, '%d-%b-%Y %h:%i %p') RegDateTime , ");
                sb.Append("  DATE_FORMAT( plo.ApprovedDate, '%d-%b-%Y %h:%i %p') ApproveDateTime , plo.ApprovedName ,");
                sb.Append(" DATE_FORMAT(plo.ApprovedDate,'%d-%b-%Y %h:%i %p')ApprovedDate , plo.ForwardDate , plo.ForwardByName ,  ");
                sb.Append("DATE_FORMAT(  plo.sradate , '%d-%b-%Y %h:%i %p')SampleReceiveArea,DATE_FORMAT(  plo.SampleCollectionDate , '%d-%b-%Y %h:%i %p')SampleCollDateTime, DATE_FORMAT( plo.`SampleReceiveDate`, '%d-%b-%Y %h:%i %p')SampleReceiveDate, ");
                sb.Append(" DATE_FORMAT(plo.ApprovedDate,'%h:%i %p')ApprovedTime , ");
                sb.Append(" plo.approved, ");
                sb.Append(" im.Name   TestDetails,otm.Name Department , ");
                ...
                sb.Append(" order by plo.SampleCollectionDate,lt.LedgerTransactionNo ) TAT Order by RegDateTime ,LabNo,Department, TestDetails ");
                        ...
                        if (dt != null && dt.Rows.Count > 0)
                        {
                            ...
                            if (Request.Form["ReportFormat"].ToString() == "1")
                            {
                                ...
                                var distinctdoctor = (from DataRow drw in dt.Rows
                                                      select new { Employee_ID = drw["Department"] }).Distinct().ToList();
                                for (int j = 0; j < distinctdoctor.Count; j++)
                                {
                                    DataTable dtdoc = dt.AsEnumerable().Where(x => x.Field<string>("Department") == Util.GetString(distinctdoctor[j].Employee_ID)).CopyToDataTable();
                                    sb.Append(" <tr> ");

                                    sb.Append(" <td colspan='15' style='font-size: 16px;font-weight: bold;padding-top: 10px;padding-bottom: 10px;text-align:left;'>Department :" + Util.GetString(dtdoc.Rows[0]["Department"].ToString()) + "</td> ");
                                    sb.Append(" </tr> ");
                                    ...
                                    for (int i = 0; i < dtdoc.Rows.Count; i++)
                                    {
                                        ...
                                        sb.AppendFormat(" <td style='width: 7%;border-bottom: 1px solid;font-size: 14px !important;word-wrap: break-word;'>{0}</td> ", Util.GetString(dtdoc.Rows[i]["AgeSex"]));
                                        ...
                                        sb.AppendFormat(" <td style='width: 7%;border-bottom: 1px solid;font-size: 14px !important;word-wrap: break-word;'>{0}</td> ", Util.GetString(dtdoc.Rows[i]["SampleReceiveDate"]));
                                        ...
                                        //sb.AppendFormat(" <td style='width: 7%;border-bottom: 1px solid;font-size: 14px !important;word-wrap: break-word;'>{0}</td> ", Util.GetString(dtdoc.Rows[i]["ForwardDate"]));
                                        sb.AppendFormat(" <td style='width: 6%;border-bottom: 1px solid;font-size: 14px !important;word-wrap: break-word;'>{0}</td> ", Util.GetString(dtdoc.Rows[i]["TestDetails"]));
                                      ...
                                      //  sb.AppendFormat(" <td style='width: 6%;border-bottom: 1px solid;font-size: 14px !important;word-wrap: break-word;'>{0}</td> ", Util.GetString(dtdoc.Rows[i]["ActualTAT1"]));
    ...
    private string MakeHeader()
    {
        ...
        sb.Append(" <td style='width: 7%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>PName</td> ");
        sb.Append(" <td style='width: 7%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>Age</td> ");
        sb.Append(" <td style='width: 7%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>Panel Name</td> ");
        sb.Append(" <td style='width: 7%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>Reg DateTime</td> ");
        sb.Append(" <td style='width: 7%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>SampleColl DateTime</td> ");
        sb.Append(" <td style='width: 7%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>Result Entered Date</td> ");
        sb.Append(" <td style='width: 7%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>Result Entered Name</td> ");
       // sb.Append(" <td style='width: 7%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>Forward Date</td> ");
        sb.Append(" <td style='width: 6%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>Investigation Name</td> ");
        sb.Append(" <td style='width: 6%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>Approve DateTime</td> ");
       // sb.Append(" <td style='width: 6%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>Selected DeliveryDate</td> ");
       // sb.Append(" <td style='width: 6%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>Actual TAT</td> ");
        sb.Append(" <td style='width: 6%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>Deviaion</td> ");
        sb.Append(" </tr> ");
        sb.Append("</table>");
        sb.Append(" </div> ");
        return sb.ToString();
