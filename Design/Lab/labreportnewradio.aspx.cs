...
public partial class Design_Lab_labreportnewradio : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            if (Util.GetString(Request.QueryString["GetReportBase64"]) != "1")
            {
              ...
              //  if (Util.GetInt(Session["RoleID"]) != 177)// open For Adminsitartor Role If DueUserInfo.RoleID != 177 ||
              //  {
                    try
                    {
                        validation objVal = new validation();
        ...
        mergeDocument();
        //System.IO.File.WriteAllText(@"F:\Lims 6.0\live_Code\itdose\ErrorLog\lab3.txt", sb.ToString());
...
_pdf = string.Concat(Resources.Resource.DocumentPath, "\\Uploaded Report") + "\\" + drAtt["FileUrl"];
                if (File.Exists(_pdf))
                {
                //    System.IO.File.WriteAllText(@"F:\Lims 6.0\live_Code\itdose\ErrorLog\po1.txt", _pdf);
                    try
                    {
        ...
        try
        {
            ...
            {
                //System.IO.File.WriteAllText(@"F:\Lims 6.0\live_Code\itdose\ErrorLog\po2.txt", "3");
                HttpContext.Current.Response.AddHeader("Content-Type", "application/pdf");
            }
    ...
    private void mergeDocument()
    {
        ...
        int pageno = 1;
        //System.IO.File.WriteAllText(@"F:\Lims 6.0\live_Code\itdose\ErrorLog\lko.txt", tempDocument.Pages.Count.ToString());
        foreach (PdfPage p in tempDocument.Pages)
        {
            ...
            PdfText reportline1 = new PdfText(PageWidth - 450, FooterHeight + 20, Util.GetString(drcurrent["ReportLine1"]), pageNumberingFont);
            p.Footer.Layout(reportline1);

            PdfText reportline2 = new PdfText(PageWidth - 450, FooterHeight + 30, Util.GetString(drcurrent["ReportLine2"]), pageNumberingFont);
    ...
    public string MakeHeader(string Header, DataRow dr_Detail)
    {
        if (dtReportHeader.Select(string.Format("PanelId='{0}'", dr_Detail["Panel_id"])).Length > 0)
            Header = dtReportHeader.Select(string.Format("PanelId='{0}'", dr_Detail["Panel_id"]))[0]["ReportHeaderRadiology"].ToString();
        else
            Header = dtReportHeader.Select("PanelId='78'")[0]["ReportHeaderRadiology"].ToString();
        ...
        if (dtObs.Select(string.Format("Test_ID='{0}' and isOrganism=1", drcurrent["Test_ID"])).Length == 0)
        {
            ...
            if (drcurrent["ispackage"].ToString() == "1" && drcurrent["ShowInReport"].ToString() == "1")
            {
                sb.Append("<tr  valign='top'>");
    ...
    private void SetHeader(PdfPage page)
    {
        ...
        if (drcurrent["isnabl"].ToString().Trim() == "1" || drcurrent["isnabl"].ToString().Trim() == "5")
        {
            ...
            if (Util.GetString(drcurrent["NablLogoPath"]).Trim() != "")
                page.Header.Layout(getPDFImagewithresize1(270, 15, 45, "App_Images" + drcurrent["NablLogoPath"].ToString().Trim()));
        }
        if (drcurrent["isnabl"].ToString().Trim() == "2" || drcurrent["isnabl"].ToString().Trim() == "5")
        {
            ...
            if (Util.GetString(drcurrent["CapLogoPath"]).Trim() != "")
                page.Header.Layout(getPDFImagewithresize(350, 15, "App_Images" + drcurrent["CapLogoPath"].ToString().Trim()));
        }
        if (drcurrent["isnabl"].ToString().Trim() == "3")
        {
            // To Set Cap Logo And Its Position
            if (Util.GetString(drcurrent["NabhLogoPath"]).Trim() != "")
                page.Header.Layout(getPDFImagewithresize(350, 15, "App_Images" + drcurrent["NabhLogoPath"].ToString().Trim()));
        }
        ...
        PdfHtml Passport = new PdfHtml(X, Y, "Passport No: " + drcurrent["PassPortNo"].ToString(), null){ FitDestWidth = true, FontEmbedding = false, BrowserWidth = HeaderBrowserWidth };
        ...
        PdfHtml Adhar = new PdfHtml(X, Y, "Aadhaar Card No: " + drcurrent["PatientIDProofno"].ToString(), null) { FitDestWidth = true, FontEmbedding = false, BrowserWidth = HeaderBrowserWidth };
        if (drcurrent["PatientIDProofno"].ToString() != "" && drcurrent["PatientIDProof"].ToString() =="1")
        {
            page.Header.Layout(Adhar);
            X += 100;
        }
        PdfHtml Nationality = new PdfHtml(X, Y, "Nationality: " + drcurrent["Nationality"].ToString(),null) { FitDestWidth = true, FontEmbedding = false, BrowserWidth = HeaderBrowserWidth };
      ...
      //  PdfHtml PureHealthID = new PdfHtml(X, Y, "Pure Health ID: " + drcurrent["PureHealthID"].ToString()) { FitDestWidth = true, FontEmbedding = false, BrowserWidth = HeaderBrowserWidth };
        PdfText PureHealthID = new PdfText(X, Y, "Pure Health ID: " + drcurrent["PureHealthID"].ToString(),  new Font("Times New Roman", 7));
    ...
    private void SetFooter(PdfPage page)
    {
        ...
        if (drcurrent["ShowSignature"].ToString() != "0")
        {
            if (!signatureonend)
            {
                ...
                {
                    string imgname = string.Format("{0}Design/opd/signature/{1}.jpg", serverPath, Util.GetString(drcurrent["ApprovedBy"]));
                    sbimg.AppendFormat("<img style='float:right;padding-right:100px;margin-top:0px;' src='{0}'", imgname);
                    sbimg.Append(" />");
                }
                sbimg.Append("</tr></td>");
            }
       ...
       // if (Util.GetString(drcurrent["type1"]).Trim() != "PCC" && Util.GetString(drcurrent["isnabl"]).Trim() == "1" || (Util.GetString(drcurrent["centreid"]).Trim() == Util.GetString(drcurrent["TestCentreID"]).Trim()) && Util.GetString(drcurrent["isnabl"]).Trim() == "1")
        ...
        page.Footer.Layout(getPDFImageforbarcode(20, FooterHeight, drcurrent["LabNo"].ToString(), "Yes"));
    ...
    private PdfImage getPDFImageforbarcode(float X, float Y, string labno,string ShowQR)
    {
        ...
        if (ShowQR == "Yes")
        {
          ...
          //  string URL = Util.GetString("http://itd-saas.cl-srv.ondgni.com/UAT_Ver1/Design/lab/labreportnew_ShortSMS.aspx?LabNo=" + Common.Encrypt(drcurrent["LabNo"].ToString()));
    ...
    private void bindReport()
    {
        ...
        sb.Append(" im.PrintSeparate,im.printsamplename,'0' ShowInReport, ");
        ...
        sb.Append(" otm.ObservationType_ID, pnl.`Panel_Code`,pnl.`ShowSignature`, ");
        sb.Append(" pnl.ReportHeaderHeight,pnl.ReportHeaderXPosition,pnl.ReportHeaderYPosition,pnl.ReportFooterHeight,pnl.ReportBackGroundImage,pnl.AAALogo,pnl.TagProcessingLabID,IFNULL(pm.Mobile, '') MobileNo,IFNULL(LEFT(pnl.Add1,30),'')ClientAddress  ");
        sb.Append(" FROM (   ");
        sb.Append(" SELECT  ");
        sb.Append("  pli.ItemId,pli.LedgerTransactionNo AS LabNo,pli.test_id PLOID,pli.laboutsrcid,IF(lt.HomeVisitBoyID='N/A','N/A',(SELECT  flm.NAME FROM feildboy_master flm WHERE flm.FeildBoyID=lt.HomeVisitBoyID)) FieldExe,pli.InterpretationID,pli.InterpretationType,");
        ...
        sb.Append(" , lt.centreid,lt.Panel_ID,Left(Concat('Dr.',UPPER(lt.`DoctorName`)),30) ReferDoctor, IF(lt.OtherReferLab='',left(lt.panelname,25),lt.OtherReferLab) Company_Name ,'' Company_code,'' Company_add,lt.`HLMPatientType`,lt.`HLMOPDIPDNo`,pli.IsSampleCollectedByPatient,IFNULL(lt.PatientType,'')PatientType");
        sb.Append(" FROM patient_labinvestigation_opd pli    ");
        sb.Append(" INNER JOIN f_ledgertransaction lt ON lt.LedgerTransactionID=pli.LedgerTransactionID  ");
        sb.Append(" AND pli.Test_ID IN ({0})  AND pli.Result_Flag = 1  AND pli.ReportType in(5) AND pli.ishold=0   ");
        sb.Append("  )t    ");
     ...
     //   sb.Append(" AND 0=(SELECT COUNT(*) FROM patient_labinvestigation_attachment_report plar WHERE plar.`Test_id`=t.`Test_ID`)  ");
        ...
        sb.Append(" INNER JOIN f_panel_master pnl ON pnl.`Panel_ID`=t.Panel_ID  INNER JOIN Patient_Master pm on pm.Patient_ID = t.Patient_ID  ");
      ...
      //  if (UserInfo.ID==1)
        //    System.IO.File.WriteAllText (@"F:\Lims 6.0\live_Code\itdose\ErrorLog\q1213223.txt", sb.ToString());
      //  if (UserInfo.ID == 1) System.IO.File.WriteAllText(@"F:\Lims 6.0\live_Code\itdose\ErrorLog\lab3.txt", sb.ToString());
        string[] tags = TestID.Replace("'", "").Split(',');
        if (isOnlinePrint != "1")
