...
public partial class Design_Lab_CGHSReport_Annexure5 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            ds = (DataSet)Session["ds5" + cmd];
            string ReportName = "";

            if (Session["ReportName5" + cmd] != null)
            {
                ReportName = Session["ReportName5" + cmd].ToString();
                Session.Remove("ReportName5" + cmd);
                 Session.Remove("ds5" + cmd);
              
                switch (ReportName)
                {
                    case "CGHSPatientReport_Annexure5":
                        {
                            obj1.Load(Server.MapPath("~/Reports/CGHSPatientReport_Annexure5.rpt"));
