...
public partial class Design_Lab_labreportnew : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (dtObs.Rows.Count > 0)
        {
            ...
            sbimg12.Append("<div style='page-break-inside:avoid;'>");
            ...
            if (!signatureonend)
            {
                ...
                if (isSign(drcurrent["ApprovedBy"].ToString()))
                {
                    ...
                    sbimg12.AppendFormat("<img style='float:left;padding-right:10px;' src='{0}'", imgname);
                    sbimg12.Append(" />");
                }
                sbimg12.Append("</tr></td>");
            }
...
_pdf = string.Concat(Resources.Resource.DocumentPath, "\\Uploaded Report") + "\\" + drAtt["FileUrl"];
    ...
    private void SetFooter(PdfPage page)
    {
        ...
        {
            page.Footer.Layout(getPDFImageforbarcode(475, 15, drcurrent["LabNo"].ToString()));
        }
    }
    ...
    private PdfImage getPDFImageforbarcode(float X, float Y, string labno)
    {
        ...
        if (AllGlobalFunction.showqrcodeinprintlabreport == "Y")
        {
            image = getqrcode("http://103.66.73.55/visiondiagnostics/Design/Lab/labreportnew.aspx?TestID=" + Util.GetString(Request.QueryString["TestID"]) + "&Phead=1");
        }
        else
        {
    ...
    private void bindReport()
    {
        ...
        sb.Append(" INNER JOIN f_ledgertransaction lt ON lt.LedgerTransactionID=pli.LedgerTransactionID  ");
        sb.Append(" AND pli.Test_ID IN ({0})  AND pli.Result_Flag = 1  AND pli.ReportType=3 AND pli.ishold=0   ");
        sb.Append("  )t    ");
