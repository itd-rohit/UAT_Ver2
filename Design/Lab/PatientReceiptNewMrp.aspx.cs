...
public partial class Design_Lab_PatientReceiptNewMrp : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {string LedgertransactionId ="";
				if(Util.GetString(Request.QueryString["IsOnline"])=="1")
		{
			Session["ID"]=" ";
			
			LedgertransactionId=Util.GetString(Request.QueryString["LabID"]);
		}
		else{
			
		          if (!string.IsNullOrEmpty(Request.QueryString["LabID"]))
                {
                   LedgertransactionId = Common.Decrypt(HttpUtility.UrlDecode(Request["LabID"]));
        ...
        try
        {
            ...
            {
                if (Util.GetInt(dtObs.Rows[0]["IsMRPBIll"]) == 0)
                    sb.Append("<td style='font-weight:bold;width:120px;text-align:right;'>Rate</td>");
                else
                    sb.Append("<td style='font-weight:bold;width:120px;text-align:right;'></td>");
            }
            else
            {
                sb.Append("<td style='font-weight:bold;width:120px;text-align:right;'>Rate</td>");
           ...
           // else
           // {
           //     sb.Append("<td style='font-weight:bold;width:80px;text-align:right;'></td>"); //Discount
           // }
           // if (Util.GetInt(dtObs.Rows[0]["HideReceiptRate"]) == 1)
           // {
            //    sb.Append("<td style='font-weight:bold;width:80px;text-align:right;'></td>");
           ...
           // else
           // {
              //  sb.Append("<td style='font-weight:bold;width:80px;text-align:right;'></td>");//Total
           // }
            sb.Append("</tr>");
            int serialno = 1;
            decimal sum = 0;
            ...
            foreach (DataRow dw in dtObs.Rows)
            {
               ...
               /// else
               // {
                   // sb.AppendFormat("<td style='text-align:right;'>{0}</td>", Math.Round(Util.GetDecimal(dw["discountamt"]), Util.GetInt(Resources.Resource.BaseCurrencyRound)), MidpointRounding.AwayFromZero);//dw["discountamt"]
               // }
               // if (Util.GetInt(dtObs.Rows[0]["HideReceiptRate"]) == 1)
               // {
               //     sb.Append("<td style='text-align:right;'></td>");
               ...
               // else
               /// {
                   // sb.AppendFormat("<td style='text-align:right;'>{0}</td>", Math.Round(Util.GetDecimal(dw["rate"]), Util.GetInt(Resources.Resource.BaseCurrencyRound)) - Math.Round(Util.GetDecimal(dw["discountamt"]), Util.GetInt(Resources.Resource.BaseCurrencyRound)), MidpointRounding.AwayFromZero);//dw["discountamt"]
               // }
                sb.Append("</tr>");
                serialno++;
            ...
            if (Util.GetInt(drcurrent["IsMRPBIll"]) == 0)
            {
                ...
                sb.AppendFormat("<td style='text-align:left;'>{0}</td>", sum, MidpointRounding.AwayFromZero);
            }
            sb.Append("</tr>");
            sb.Append("<tr>");
             ...
             if (Util.GetInt(drcurrent["IsMRPBIll"]) == 0)
            {
                if (discountsum != 0)
                {
                    adjustment = Util.GetFloat(sum) - Util.GetFloat(discountsum);
                }
                else
                {
                     adjustment = Util.GetFloat(sum);
                }



                sb.Append("<td colspan='2' style='text-align:right;'><b>Total Paid Amount :</b></td>");
                sb.AppendFormat("<td style='text-align:left;'>{0}</td>", Math.Round(Util.GetDecimal(sum), Util.GetInt(Resources.Resource.BaseCurrencyRound)), MidpointRounding.AwayFromZero);//drcurrent["adjustment"]
            }
            sb.Append("</tr>");
            ...
            if (Util.GetInt(drcurrent["IsMRPBIll"]) == 0)
            {
                ...
                sb.AppendFormat("<td style='text-align:left;'>{0}</td>", Math.Round(Util.GetDecimal(0.00), Util.GetInt(Resources.Resource.BaseCurrencyRound)), MidpointRounding.AwayFromZero);//dueamount
            }
            sb.Append("</tr>");
    ...
    private void BindData()
    {
		...
		if(Util.GetString(Request.QueryString["IsOnline"])=="1")
		{
			...
			labid=Util.GetString(Request.QueryString["LabID"]);
		}
		else{
		...
		if(Util.GetString(Request.QueryString["IsOnline"])=="1")
		{
			  string  CentreType = Util.GetString(MySqlHelper.ExecuteScalar(con, CommandType.Text, " SELECT PatientType FROM f_ledgertransaction WHERE LedgertransactionID=@LedgerTransactionID  LIMIT 1 ",
               new MySqlParameter("@LedgerTransactionID", labid)));
			     Session["CentreType"]=CentreType;
		}
        ...
        try
        {
            ...
            dtSettlement = MySqlHelper.ExecuteDataset(con, CommandType.Text, "SELECT ReceiptNo,PaymentMode,S_Notation,S_Currency,Amount,DATE_FORMAT(CreatedDate,'%d-%m-%Y') CreatedDate,CreatedBy UserName,CurrencyRoundDigit,S_Amount FROM f_receipt WHERE `LedgerTransactionID`=@LedgerTransactionID and iscancel=0",
            ...
            sb.Append(" pm.`Age`,pm.`Gender`,pm.`Mobile`, lt.`PanelName`,IF(lt.`DoctorName`='SELF',lt.`DoctorName`,CONCAT('Dr.', lt.`DoctorName`))Doctorname, ");
            ...
            sb.AppendFormat(" itm.TestCode ,SUM(plo.MRP*plo.Quantity) as  rate,IF('{0}'=0,SUM(plo.`Amount`),SUM(plo.MRP))Amount,(plo.rate*plo.Quantity -SUM(plo.`Amount`)) as `DiscountAmt`, DATE_FORMAT(plo.deliverydate,'%d-%b-%Y %h:%i%p')deliveryDate,  lt.`GrossAmount`, ", MRPBill);//(plo.rate*plo.Quantity -SUM(plo.`Amount`)) as `DiscountAmt`,
            ...
            sb.Append(" ,ifnull(frt1.ItemCode,'')ItemCode");//ifnull(prm.PROName,'') PROName,
         ...
         //   System.IO.File.WriteAllText(@"D:\Live_Opams\Live_Code\OWD\ErrorLog\ruby999.txt", labid);
            ...
            sb.Append(" SELECT ");
            sb.Append(" plo.IsPackage, itm.TypeName ItemName ,itm.TestCode ,plo.rate,plo.`Amount`,plo.`DiscountAmt`, ");// 
