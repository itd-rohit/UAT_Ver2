...
public partial class Design_Store_StockStatusReportPDF : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                if (centrename != Util.GetString(dw["Storelocation"]))
                {
                    sb.Append("<tr>");
                    sb.Append("<td colspan='10' style='font-weight:bold;background-color:lightgray;padding:8px;'>Location : " + Util.GetString(dw["Storelocation"]) + "</td>");
                    sb.Append("</tr>");
                    centrename = Util.GetString(dw["Storelocation"]);
                }
                sb.Append("<tr>");
                sb.Append("<td style='width:200px; word-wrap:break-word;'>" + Util.GetString(dw["itemname"]) + "</td>");
                sb.Append("<td style='width:100px;'>" + Util.GetString(dw["Manufacture"]) + "</td>");
                sb.Append("<td style='width:60px;'>" + Util.GetString(dw["PackSize"]) + "</td>");
                sb.Append("<td style='width:70px;'>" + Util.GetString(dw["barcodeno"]) + "</td>");
                sb.Append("<td style='width:70px;'>" + Util.GetString(dw["batchnumber"]) + "</td>");
                sb.Append("<td style='width:60px;'>" + Util.GetString(dw["expirydate"]) + "</td>");
                sb.Append("<td style='width:80px;text-align:right;'>" + Math.Round(Util.GetDecimal(dw["stockinhandqty"]), 2) + "</td>");
                sb.Append("<td style='width:80px;'>" + Util.GetString(dw["consumeunit"]) + "</td>");
                sb.Append("<td style='width:80px;text-align:right;'>" + Math.Round(Util.GetDecimal(dw["stockamount"]), 2) + "</td>");
                sb.Append("</tr>");
    ...
    private string MakeHeader()
    {
        ...
        Header.Append("<tr style='border-bottom:1px solid black;'><td colspan='4' align='center' style='border:1px solid black;font-size:17px;font-weight:bold;padding:5px;'>Stock Status Report</td></tr>");
        ...
        Header.Append("<tr>");
        Header.Append("<td valign='top' style='font-weight:bold;width:200px;'>ItemName</td>");
        Header.Append("<td valign='top' style='font-weight:bold;width:100px;'>Manufacture</td>");
        Header.Append("<td valign='top' style='width:60px;font-weight:bold;'>Pack Size</td>");
        Header.Append("<td valign='top' style='width:70px;font-weight:bold;'>Barcode No</td>");
        Header.Append("<td valign='top' style='width:70px;font-weight:bold;'>Batch No</td>");
        Header.Append("<td valign='top' style='width:60px;font-weight:bold;'>Expiry Date</td>");
        Header.Append("<td valign='top' style='width:80px;font-weight:bold; text-align:right;'>Stock In Hand</td>");
        Header.Append("<td valign='top' style='width:80px;font-weight:bold;'>Consume Unit</td>");
        Header.Append("<td valign='top' style='width:80px;font-weight:bold; text-align:right;' >Stock Amount</td>");
        Header.Append("</tr>");
        Header.Append("</table></div>");
    ...
    private void BindData()
    {
        ...
        try
        {
            ...
            dtObs = Store_ReportQuery.StockStatusReport(Request.Form["ItemID"], Request.Form["ManufactureID"], Request.Form["MacID"],Request.Form["LocationID"]);
        }
        catch (Exception ex)
        {
