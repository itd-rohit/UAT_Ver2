...
public partial class Design_Store_PODGenerate : System.Web.UI.Page
{
    ...
    public static string SearchData(string location, string fromdate, string todate, string supplier, string ponumber, string invoiceno, string grnno, string grnstatus)
    {
        ...
        sb.Append(" vm.SupplierName,DATE_FORMAT(lt.datetime,'%d-%b-%Y')GRNDate,lt.NetAmount,lt.GrossAmount,lt.DiscountOnTotal,lt.TaxAmount,lt.podgroup, ");
        ...
        sb.Append(" and if(lt.Ispodgenerate = 0,lt.datetime>='2019-01-15 00:00:00', 1=1 ) ");
    ...
    public static string Post(List<poddetails> objpoddetails)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    string str = "update st_ledgertransaction set PODnumber ='" +"POD_"+ objpoddetails[i].grn_no + "',PODGroup='"+indentno+"',Ispodgenerate =1,PODcreated_by='" + UserInfo.ID + "',PODgendate=now() where LedgerTransactionID=" + objpoddetails[i].LedgerTransactionID + " ";
                    // StockReports.ExecuteDML(str);
                    MySqlHelper.ExecuteNonQuery(tnx, CommandType.Text, str);


                    MySqlHelper.ExecuteNonQuery(tnx, CommandType.Text, "INSERT INTO st_pod_details (LedgerTransactionID,LedgerTransactionNo,locationid,GrossAmount,DiscountOnTotal,TaxAmount,NetAmount,InvoiceNo,PODnumber,PODcreated_by,PODgendate,PODGroup) values(  @LedgerTransactionID,@LedgerTransactionNo,@locationid,@GrossAmount,@DiscountOnTotal,@TaxAmount,@NetAmount,@InvoiceNo,@PODnumber,@PODcreated_by,now(),@PODGroup)",
                           ...
                           new MySqlParameter("@PODnumber", "POD_"+objpoddetails[i].grn_no),
                             new MySqlParameter("@PODcreated_by", UserInfo.ID),
                             new MySqlParameter("@PODGroup",indentno )
    ...
    public static string transfer(List<poddetails> objpoddetails, string curriername, string consinment, string courierdate)
    {
        //check permission 
        string rt = "";
        string dtispermit = StockReports.ExecuteScalar("SELECT IFNULL(employeeidfrom,0) FROM st_pod_employeetagging WHERE  isactive=1 AND employeeidfrom='" + UserInfo.ID + "' ");
        if (dtispermit == "" || dtispermit == "0")
        {
            return rt = "2";
        }
        ...
        try
        {
            ...
            for (int i = 0; i < objpoddetails.Count; i++)
            {
                MySqlHelper.ExecuteNonQuery(tnx, CommandType.Text, "insert into st_PodTransfer (PodNumber,FromEmployeeId,Status,IsCurrent)values(@PodNumber,@FromEmployeeId,@Status,@IsCurrent)",
                    new MySqlParameter("@PodNumber",objpoddetails[i].podnumber),
                    new MySqlParameter("@FromEmployeeId",UserInfo.ID),
                    new MySqlParameter("@Status","Transfer"),
                    new MySqlParameter("@IsCurrent",1));
