<%@ Page ClientIDMode="Static" Language="C#" MasterPageFile="~/Design/DefaultHome.master" AutoEventWireup="true" CodeFile="CampConfigurationMaster.aspx.cs" Inherits="Design_Camp_CampConfigurationMaster" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div id="Pbody_box_inventory">
        <div class="POuter_Box_Inventory" style="text-align: center">
            <b>Camp Configuration Count Master</b>
            <br />
        ...
        <div class="POuter_Box_Inventory" style="text-align: center; display: none" id="divSave">
            <input type="button" id="btnSave" value="Save" onclick="$saveCampConfig()" class="savebutton" />
        </div>
    </div>
    ...
    <script type="text/javascript">
        ...
        bindClient = function () {
            jQuery('#lstClient option').remove();
            jQuery('#lstClient').multipleSelect("refresh");
            serverCall('CampConfigurationMaster.aspx/bindClient', { BusinessZoneID: jQuery("#ddlBusinessZone").val(), StateID: jQuery("#ddlState").val(), ClientTypeID: jQuery('#lstClientType').val().toString(), TagBusinessLabID: jQuery('#lstTagBusinessLab').val().toString() }, function (response) {
        ...
        $bindDetail = function (SearchType) {
            ...
            serverCall('CampConfigurationMaster.aspx/bindDetail', { FinancialYear: jQuery("#ddlFinacial").val(), ClientID: ClientID, SearchType: SearchType }, function (response) {
                ...
                if ($responseData.status) {
                    ...
                    if (userContext.SearchType == 0) {
                        ...
                        hot2 = new Handsontable(container2, {
                            data: CampConfigData,
                            colHeaders: [
                                  "S.No.", "TagBusinessLab", "ClientType", "ClientCode", "ClientName", "April", "May", "June", "July", "August", "September", "October", "November", "December", "January", "February", "March", "Total"
                            ],
                            columns: [
                                 { renderer: AutoNumberRenderer, width: '60px', readOnly: true },
                                 { data: 'TagBusinessLab', readOnly: true, className: "htLeft htMiddle", renderer: approved },
                                 { data: 'ClientType', readOnly: true, className: "htLeft htMiddle", renderer: approved },
                                 { data: 'ClientCode', readOnly: true, className: "htLeft htMiddle", renderer: approved },
                                 { data: 'ClientName', readOnly: true, className: "htLeft htMiddle", renderer: approved },
                                 { data: 'Apr', type: 'numeric', format: '8', readOnly: false, allowInvalid: false, renderer: CampConfigTotal },
                                 { data: 'May', type: 'numeric', format: '8', readOnly: false, allowInvalid: false, renderer: CampConfigTotal },
                                 { data: 'Jun', type: 'numeric', format: '8', readOnly: false, allowInvalid: false, renderer: CampConfigTotal },
                                 { data: 'Jul', type: 'numeric', format: '8', readOnly: false, allowInvalid: false, renderer: CampConfigTotal },
                                 { data: 'Aug', type: 'numeric', format: '8', readOnly: false, allowInvalid: false, renderer: CampConfigTotal },
                                 { data: 'Sep', type: 'numeric', format: '8', readOnly: false, allowInvalid: false, renderer: CampConfigTotal },
                                 { data: 'Oct', type: 'numeric', format: '8', readOnly: false, allowInvalid: false, renderer: CampConfigTotal },
                                 { data: 'Nov', type: 'numeric', format: '8', readOnly: false, allowInvalid: false, renderer: CampConfigTotal },
                                 { data: 'Dec', type: 'numeric', format: '8', readOnly: false, allowInvalid: false, renderer: CampConfigTotal },
                                 { data: 'Jan', type: 'numeric', format: '8', readOnly: false, allowInvalid: false, renderer: CampConfigTotal },
                                 { data: 'Feb', type: 'numeric', format: '8', readOnly: false, allowInvalid: false, renderer: CampConfigTotal },
                                 { data: 'Mar', type: 'numeric', format: '8', readOnly: false, allowInvalid: false, renderer: CampConfigTotal },
                                 { data: 'Total', type: 'numeric', format: '96', readOnly: true, allowInvalid: false, renderer: CampTotal },
                            ],
                            stretchH: "all",
                            fixedColumnsLeft: 5,
                            autoWrapRow: false,
                            ...
                            colWidths: [30, 180, 120, 80, 220, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 80, 100],
        ...
        CampConfigTotal = function (instance, td, row, col, prop, value, cellProperties) {
        ...
        CampTotal = function (instance, td, row, col, prop, value, cellProperties) {
            Handsontable.NumericRenderer.apply(this, arguments);
            td.style.fontWeight = 'bold';
            ...
            if (row === CampConfigData.length - 1) {
                ...
                if (instance.getDataAtCell(row, 5) != "")
                    Apr = parseInt(instance.getDataAtCell(row, 5));
                if (instance.getDataAtCell(row, 6) != "")
                    May = parseInt(instance.getDataAtCell(row, 6));
                if (instance.getDataAtCell(row, 7) != "")
                    Jun = parseInt(instance.getDataAtCell(row, 7));
                if (instance.getDataAtCell(row, 8) != "")
                    Jul = parseInt(instance.getDataAtCell(row, 8));
                if (instance.getDataAtCell(row, 9) != "")
                    Aug = parseInt(instance.getDataAtCell(row, 9));
                if (instance.getDataAtCell(row, 10) != "")
                    Sep = parseInt(instance.getDataAtCell(row, 10));
                if (instance.getDataAtCell(row, 11) != "")
                    Oct = parseInt(instance.getDataAtCell(row, 11));
                if (instance.getDataAtCell(row, 12) != "")
                    Nov = parseInt(instance.getDataAtCell(row, 12));
                if (instance.getDataAtCell(row, 13) != "")
                    Dec = parseInt(instance.getDataAtCell(row, 13));
                if (instance.getDataAtCell(row, 14) != "")
                    Jan = parseInt(instance.getDataAtCell(row, 14));
                if (instance.getDataAtCell(row, 15) != "")
                    Feb = parseInt(instance.getDataAtCell(row, 15));
                if (instance.getDataAtCell(row, 16) != "")
                    Mar = parseInt(instance.getDataAtCell(row, 16));
    ...
    <script type="text/javascript">
        $saveCampConfig = function () {
            ...
            if (CampConfigData.length > 0) {
                ...
                serverCall('CampConfigurationMaster.aspx/SaveCampConfig', { CampConfigDetail: CampConfigDataDetail, FinacialYearDetail: jQuery('#spnFinacialYear').text() }, function (response) {
                    var $responseData = JSON.parse(response);
                    if ($responseData.status) {
