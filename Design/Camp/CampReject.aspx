<%@ Page ClientIDMode="Static" Language="C#" MasterPageFile="~/Design/DefaultHome.master" AutoEventWireup="true" CodeFile="CampReject.aspx.cs" Inherits="Design_Camp_CampReject" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
             ...
             <div class="col-md-2">
                 <asp:DropDownList ID="ddlStatus" runat="server"  CssClass="ddlStatus chosen-select">
                         ...
                         <%--  <asp:ListItem  Value="0" Selected="True" Text="Pending"></asp:ListItem>--%>
                           <asp:ListItem  Value="1" Selected="True" Text="Created"></asp:ListItem>
        ...
        <div class="row">
             <div class="col-md-4">&nbsp;</div>
                  <%--<div class="col-md-1" style="background-color:#B0C4DE;height: 20px;width:2%;border:1px solid black;cursor:pointer;" onclick="searchCampRequest('2')">
                    ...
                    <div class="col-md-2" style="text-align:left"> Pending
                 </div> --%>               
    ...
    <input type="button" value="Search" class="searchbutton" onclick="searchCampRequest(0)" id="btnSearch" />
     ...
     <div id="divCampTest" class="modal fade">
        ...
        <div id="divCampRejectReason" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content" style="min-width: 40%; max-width: 42%">
                <div class="modal-header">
                    <div class="row">
                        <div class="col-md-12" style="text-align: left">
                            <h4 class="modal-title">Camp Detail</h4>
                        </div>
                        <div class="col-md-12" style="text-align: right">
                            <em><span style="font-size: 7.5pt; color: #0000ff;">Press esc or click
                                <button type="button" class="closeModel" onclick="$closeRejectModel()" aria-hidden="true">&times;</button>to close</span></em>
                        </div>
                    </div>
                </div>
                <div class="modal-body">
                ...
                <div class="col-md-18 ">
                    <asp:TextBox ID="txtRejectReason" runat="server" MaxLength="50" CssClass="requiredField"></asp:TextBox>
                    </div>
                </div>
                    <div class="row" style="text-align:center">
                         <div class="col-md-24 " style="text-align:center">
                             <input type="button" id="btnReject" onclick="$campRejected()" />
                         </div>
                    </div>
                     </div>
                <div class="modal-footer">
                    <button type="button" onclick="$closeRejectModel()">Close</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script type="text/javascript">
        $campRejected = function () {
            if (jQuery.trim( jQuery("#txtRejectReason").val()) == "") {
                toast('Error', 'Please Enter Reject Reason');
                jQuery("#txtRejectReason").focus();
                return;
            }
            jQuery("#btnReject").val('Submitting...').attr('disabled', 'disabled');

            serverCall('CampReject.aspx/ApproveCampRequest', { ID: jQuery("#spnCampID").text(), RejectReason: jQuery("#txtRejectReason").val() }, function (response) {
                ...
                jQuery("#btnReject").val('Reject').removeAttr('disabled');
            });
        }
        $closeRejectModel = function () {
            jQuery('#divCampRejectReason').hideModel();
        ...
        function onKeyDown(e) {
            if (e && e.keyCode == Sys.UI.Key.esc) {
               ...
               else if (jQuery('#divCampRejectReason').is(':visible')) {
                    $closeRejectModel();
                }
            }
        }
        ...
        $getCampTestDetail = function (rowID) {
            ...
            serverCall('CampReject.aspx/CampTestDetail', { ID: jQuery(rowID).closest('tr').find("#tdID").text() }, function (response) {
        ...
        function searchCampRequest(searchType) {
            ...
            serverCall('CampReject.aspx/SearchCampRequest', { fromDate: jQuery('#txtFromDate').val(), toDate: jQuery('#txtToDate').val(), searchType: searchType, status: jQuery('#ddlStatus').val(), TypeID: jQuery('#ddlType').val() }, function (response) {
                ...
                if (Edata.length == 0) {
                    ...
                    jQuery('#lbltotal').text(Edata.length);
        ...
        $CampReject = function (rowID) {
            jQuery('#lblCampName').text(jQuery(rowID).closest('tr').find("#tdCampName").text());
            jQuery('#spnCampID').text(jQuery(rowID).closest('tr').find("#tdID").text());
            jQuery('#txtRejectReason').val();
            jQuery("#btnReject").val('Reject').removeAttr('disabled');
            jQuery('#divCampRejectReason').show();
        }
         </script>
    ...
    style="width:100%;border-collapse:collapse;">
		...
		<tr id="Header">
            ...
            <th class="GridViewHeaderStyle" scope="col" style="width:80px;">Camp Type</th>
            <th class="GridViewHeaderStyle" scope="col" style="width:120px;">Camp Date</th>          
            ...
            <th class="GridViewHeaderStyle" scope="col" style="width:60px;">Reject</th>          
 ...
 <# 
                        if(objRow.IsActive=="1" && objRow.RegCount=="0"){#>
                       <input type="button" id="btnApproved" class="ItDoseButton"  value="Reject"  onclick="$CampReject(this)"  />                                                              
<#}#>
                        </td>    
                        
                        <td class="GridViewLabItemStyle" id="tdID" style="display:none" ><#=objRow.ID#></td>  
                        <td class="GridViewLabItemStyle"  style="text-align:center">
                            <img src="../../App_Images/view.GIF" style="cursor:pointer" onclick="$getCampTestDetail(this)"  alt=""/>
                            </td>       
    ...
    <script type="text/javascript">
        function CampApprove(rowid) {
            jQuery(rowid).closest('tr').find("#btnApproved").val('Submitting...').attr('disabled', 'disabled');

            serverCall('CampReject.aspx/ApproveCampRequest', { ID: jQuery(rowid).closest('tr').find("#tdID").text() }, function (response) {
                var $responseData = JSON.parse(response);
                if ($responseData.Status) {
                    jQuery(rowid).closest('tr').find("#btnApproved").hide();
                    window.open('CampRequestMaster.aspx?ID=' + $responseData.response + '');
                }
                else {
                    toast("Error", $responseData.response, "");
                }
        ...
        getCampDetail = function () {
            jQuery('#ddlType option:selected').text('<%=UserInfo.CentreType%>');
          
            jQuery('#ddlType').attr('disabled', 'disabled');
            jQuery('#ddlType,#ddlStatus').trigger('chosen:updated');
            jQuery("#ddlType,#ddlStatus").chosen("destroy").chosen({ width: '100%' });
        }
    </script>
