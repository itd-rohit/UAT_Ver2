...
public partial class Design_Employee_EmployeeRegistration : System.Web.UI.Page
{
        ...
        try
        {
            using (DataTable dt = MySqlHelper.ExecuteDataset(con, CommandType.Text, "select *,IF(DOB='0001-01-01','', DATE_FORMAT(DOB,'%d-%b-%Y'))DateOfBirth from employee_master where Employee_ID=@empid", new MySqlParameter("@empid", lblEmployee_ID.Text.Trim())).Tables[0])
            {
                if (dt.Rows.Count > 0)
                {
                    ...
                    //BindPRO();
                    ddlPROHead.SelectedValue = dt.Rows[0]["PROID"].ToString();
    ...
    public static string SaveData(string Title, string Name, string HouseNo, string Street, string Locality, string City, string Pincode, string OHouse, string OStreet, string OLocality, string OCity, string OPincode, string Father, string Mother, string ESI, string EPF, string PAN, string Passport, string DOB, string Qualification, string Email, string AllowSharing, string STD, string Phone, string Mobile, string BloodGroup, string StartDate, string Username, string Pwd, string ConfirmPwd, string Dept, string Centre, string Role, string Login, int defaultCentreID, string Designation, int defaultRoleID, int ApproveSpecialRate, int AmrValueAccess, int DesignationID, int ValidateLogin, int IsMobileAccess, int PROID, string IsHideRate, string IsEditMacReading, int IsSampleLogisticReject, int GlobalReportAccess,string PROHeadID)
    {
        ...
        try
        {
         ...
         //   MySqlHelper.ExecuteNonQuery(MySqltrans, CommandType.Text, "Update Employee_Master set PROID='" + PROHeadID  + "' where Employee_Id='"+ Employee_id +"'");
            ...
            else
            {
                ...
                sb_1.Append("  values('','Employee Update','','" + Name + "','" + Util.GetString(HttpContext.Current.Session["ID"]) + "','" + Util.GetString(HttpContext.Current.Session["LoginName"]) + "',NOW(),'" + Util.GetString(HttpContext.Current.Session["RoleID"]) + "','" + Util.GetString(UserInfo.Centre) + "','Add  New Employee Name is" + Name + "','" + StockReports.getip() + "',64);  ");
                MySqlHelper.ExecuteNonQuery(MySqltrans, CommandType.Text, sb_1.ToString());
                MySqltrans.Commit();
    ...
    public static string UpdateData(string Title, string Name, string HouseNo, string Street, string Locality, string City, string Pincode, string OHouse, string OStreet, string OLocality, string OCity, string OPincode, string Father, string Mother, string ESI, string EPF, string PAN, string Passport, string DOB, string Qualification, string Email, string AllowSharing, string STD, string Phone, string Mobile, string BloodGroup, string StartDate, string Username, string Pwd, string ConfirmPwd, string Dept, string Centre, string Role, string Login, int defaultCentreID, string Designation, int defaultRoleID, int ApproveSpecialRate, int AmrValueAccess, int DesignationID, string Employee_ID, int ValidateLogin, int IsMobileAccess, int PROID, int IsHideRate, int IsEditMacReading, int IsSampleLogisticReject, int GlobalReportAccess,string PROHeadID)
    {
        ...
        try
        {
            ...
            //  Employee  Log
            StringBuilder sb_1 = new StringBuilder();
            sb_1.Append("SELECT  em.Employee_ID,Title,em.`Name`,fl.UserName,fl.PASSWORD,em.Mobile,em.Designation,GROUP_CONCAT(DISTINCT rm.`RoleName`)RoleName,");
            sb_1.Append(" AccessDepartment,GROUP_CONCAT(DISTINCT cm.centre)centre, em.House_No,em.Street_Name,em.Locality,em.City,em.Pincode,em.PHouse_No ");
            sb_1.Append(" ,em.PLocality, em.DOB,em.Qualification,em.BloodGroup,em.FatherName,em.MotherName,em.ESI_No,em.EPF_No,em.PAN_No,em.PassportNo,em.Email, em.StartDate,em.AllowSharing,em.empgroupid ");
            sb_1.Append(" ,em.CreatedByID,em.ApproveSpecialRate, em.AmrValueAccess,em.DesignationID,em.ValidateLogin,em.IsMobileAccess,em.PROID,em.IsHideRate,em.IsEditMacReading,em.IsSampleLogisticReject,em.CreatedBy,em.GlobalReportAccess  ");
            sb_1.Append(" FROM  employee_master em ");
            sb_1.Append(" INNER JOIN f_login fl ON fl.`EmployeeID`=em.Employee_ID  INNER JOIN f_rolemaster rm ON rm.`id`=fl.`RoleID` ");
            sb_1.Append(" INNER JOIN centre_master cm ON cm.`CentreID`=fl.`CentreID`   WHERE em.Employee_ID='" + Employee_ID.Trim() + "' and em.`IsActive`=1 AND fl.`Active`=1 GROUP BY em.`Employee_ID` ORDER BY em.`Name` ");
            DataTable dt_LTD_1 = StockReports.GetDataTable(sb_1.ToString());

            string holdlog1 = "";

            string[] newlog11 = (MySqlHelper.ExecuteScalar(MySqltrans, CommandType.Text, "select AccessDepartment from employee_master WHERE Employee_ID='" + Employee_ID.Trim() + "' ")).ToString().Split(',');

            foreach (var item in newlog11)
            {
                if (holdlog1 == "")
                    holdlog1 += item;
                else
                ...
                string arr1 = Util.GetString(MySqlHelper.ExecuteScalar(MySqltrans, CommandType.Text, "SELECT GROUP_CONCAT(DisplayName) FROM  f_subcategorymaster sb  WHERE sb.id  in ('" + holdlog1.TrimEnd(',') + "') "));
          ...
          //  MySqlHelper.ExecuteNonQuery(MySqltrans, CommandType.Text, "Update Employee_Master set PROID='" + PROHeadID + "' where Employee_Id='" + Employee_ID + "'");
            ...
            else
            {
                ...
                string[] newlog1 = (MySqlHelper.ExecuteScalar(MySqltrans, CommandType.Text, "select AccessDepartment from employee_master WHERE Employee_ID='" + Employee_ID.Trim() + "' ")).ToString().Split(',');


                foreach (var item in newlog1)
                {
                    holdlog += item + ",";
                }
                string arr = Util.GetString(MySqlHelper.ExecuteScalar(MySqltrans, CommandType.Text, "SELECT GROUP_CONCAT(DisplayName) FROM  f_subcategorymaster sb  WHERE sb.id  in (" + holdlog.TrimEnd(',') + ") "));
                //  Employee  Log
                ...
                DataTable dt_LTD_2 = MySqlHelper.ExecuteDataset(MySqltrans, CommandType.Text, sb_1.ToString()).Tables[0];
                ...
                for (int i = 0; i < dt_LTD_1.Columns.Count; i++)
                {
                    ...
                    {                                              
                             if (i == 8)
                            {
                                ...
                                sb_1.Append("  values('Employee Update','" + arr1 + "','" + arr + "','" + Util.GetString(HttpContext.Current.Session["ID"]) + "','" + Util.GetString(HttpContext.Current.Session["LoginName"]) + "',NOW(),'" + Util.GetString(HttpContext.Current.Session["RoleID"]) + "','" + Util.GetString(UserInfo.Centre) + "','Change " + dt_LTD_2.Rows[0]["Name"] + " " + _ColumnName + " from " + Util.GetString(dt_LTD_1.Rows[0][i]) + " to " + Util.GetString(dt_LTD_2.Rows[0][i]) + "','" + StockReports.getip() + "',64);  ");
                                MySqlHelper.ExecuteNonQuery(MySqltrans, CommandType.Text, sb_1.ToString());
                            }
                            else
                            {
                                ...
                                sb_1 = new StringBuilder();
                                sb_1.Append("  INSERT INTO `patient_labinvestigation_opd_update_status`(`Status`,OLDNAME,`NEWNAME`,UserID,UserName,dtEntry,RoleID,CentreID,Remarks,IpAddress,StatusID) ");
                                sb_1.Append("  values('Employee Update','" + Util.GetString(dt_LTD_1.Rows[0][i]) + "','" + Util.GetString(dt_LTD_2.Rows[0][i]) + "','" + Util.GetString(HttpContext.Current.Session["ID"]) + "','" + Util.GetString(HttpContext.Current.Session["LoginName"]) + "',NOW(),'" + Util.GetString(HttpContext.Current.Session["RoleID"]) + "','" + Util.GetString(UserInfo.Centre) + "','Change " + dt_LTD_2.Rows[0]["Name"] + " " + _ColumnName + " from " + Util.GetString(dt_LTD_1.Rows[0][i]) + " to " + Util.GetString(dt_LTD_2.Rows[0][i]) + "','" + StockReports.getip() + "',64);  ");
                                MySqlHelper.ExecuteNonQuery(MySqltrans, CommandType.Text, sb_1.ToString());
                            }
                        }
    ...
    public static string SaveLogin(MySqlTransaction MySqltrans, string EmployeeID, string Username, string Pwd, string ConfirmPwd, string Centre, string Role, string Login, int defaultCentreID, int defaultRoleID)
    {
        ...
        {
            rslt = -3;
            return "Username and Password both must be entered";
        }
