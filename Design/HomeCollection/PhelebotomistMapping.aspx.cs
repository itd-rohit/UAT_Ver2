...
public partial class Design_HomeCollection_PhelebotomistMapping : System.Web.UI.Page
{
    ...
    public static string bindState(string BusinessZoneID)
    {
        ...
        try
        {
            ...
            {
                using (DataTable dt = MySqlHelper.ExecuteDataset(con, CommandType.Text, " SELECT ID,State FROM state_master  Where  IsActive=1 ORDER BY state ",
                   new MySqlParameter("@BusinessZoneID", BusinessZoneID)).Tables[0])
                {
                    retr = JsonConvert.SerializeObject(dt);
                }
    ...
    public static string BindPheleboTable(string Pincode, string Pheleboid, string CityId)
    {
        ...
        try
        {
            ...
            sb.Append("  INNER JOIN f_locality fl ON fl.ID=rpm.localityid and fl.Pincode=@Pincode and  fl.cityid in (" + CityId + ") ");
            sb.Append("  LEFT JOIN  " + Util.getApp("HomeCollectionDB") + ".hc_phlebotomistmaster pm ON pm.PhlebotomistID=rpm.PhlebotomistID  AND pm.PhlebotomistID = @PhlebotomistID    order by fl.Pincode,fl.`NAME` ");

            DataTable dt =  MySqlHelper.ExecuteDataset(con, CommandType.Text, sb.ToString(),
             new MySqlParameter("@Pincode", Pincode), new MySqlParameter("@PhlebotomistID", Pheleboid)).Tables[0];
            return JsonConvert.SerializeObject(dt);
    ...
    public static MySqlDbType Pheleboid { get; set; }
}