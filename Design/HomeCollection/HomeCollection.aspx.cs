    ...
    public static string savedata(List<Insert_PreBooking> datatosave, string AppDateTime, int updatepatient, int HardCopyRequired, int PhlebotomistID, string Latitude, string Longitude, string ispermanetaddress, string ReceiptRequired, string Alternatemobileno, string Client, string Paymentmode, string SourceofCollection, string Phelebotomistname, string emailidpcc, string centrename, string RouteName, string RouteID, string deliverych, string endtime, string oldprebookingid, string hcrequestid, string followupcallid, string phelboshare)
    {
        ...
        try
        {
         ...
         //   String[] puplist = Util.getApp("PUPListForNewSMS").Split(',');
            ...
            String SMSTemplate = Util.GetString(MySqlHelper.ExecuteScalar(tnx, CommandType.Text, "Select Template from sms_configuration sc inner join sms_configuration_client scc on sc.id=scc.smsconfigurationid where sc.IsActive=1  and sc.id=25 and IsPatient=1"  //and scc.Panel_ID=@PanelID
            ...
            // Email To Patient
             String EmailTemplate = Util.GetString(MySqlHelper.ExecuteScalar(tnx, CommandType.Text, "Select Concat(Subject,'#',Template) from Email_configuration sc inner join Email_configuration_client scc on sc.id=scc.emailconfigurationid where sc.IsActive=1 and sc.id=9 and IsPatient=1 LIMIT 1"));
            //Whatsapp To Patient
            if (EmailTemplate != "" && datatosave[0].Email != string.Empty)
            {
                smsText = new StringBuilder();
                smsText.Append(EmailTemplate.Split('#')[1]);
                smsText = new StringBuilder();
                ...
                sbSMS.Append(" values(@PreBookingID, @EmailID,'','',@Subject,@EmailBody,'Home Collection','',now(),@EntryByID,@EntryByName) ");
                MySqlHelper.ExecuteNonQuery(tnxhc, CommandType.Text, sbSMS.ToString(),
                            new MySqlParameter("@PreBookingID", PreBookingID),
