...
public partial class Design_OPD_OutStandingReportPdf : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                ...
                if (Request.Form["ReportType"].ToString() == "0")
                {
                    ...
                    sb.Append("  SELECT DATE_FORMAT(lt.date,'%d-%m-%Y')DATE,lt.LedgerTransactionNo,lt.`PName`,lt.`Age`,lt.`Gender`,lt.`Panel_ID`,lt.`PanelName`, ");                    ;
                    sb.Append("  ROUND(lt.GrossAmount,2) GrossAmount,ROUND(lt.DiscountOnTotal,2)DiscountAmt, ROUND(lt.NetAmount,2)NetAmount,");
                    sb.Append("  ROUND(lt.NetAmount- SUM(IFNULL(r.Amount,0)),2)NetBalance,SUM(IFNULL(r.Amount,0)) ReceivedAmt, ");                   

                    sb.Append("  SUM(IF(IFNULL(r.`PaymentModeID`,0)=1,IFNULL(r.Amount,0),0))CashAmt, ");
                    sb.Append("  SUM(IF(IFNULL(r.`PaymentModeID`,0)=2,IFNULL(r.Amount,0),0))ChequeAmt, ");
                    sb.Append(" SUM(IF(IFNULL(r.`PaymentModeID`,0)=3,IFNULL(r.Amount,0),0))CardAmt, ");
                    sb.Append("  SUM(IF(IFNULL(r.`PaymentModeID`,0)=10,IFNULL(r.Amount,0),0))MobileWallet, ");
                    sb.Append(" SUM(IF(IFNULL(r.`PaymentModeID`,0)=5,IFNULL(r.Amount,0),0))DebitCardAmt,    ");
                    sb.Append("  SUM(IF(IFNULL(r.`PaymentModeID`,0)=6,IFNULL(r.Amount,0),0))OnlinePayment  ");
                    sb.Append("  FROM f_ledgertransaction lt ");
                    sb.Append("  LEFT JOIN f_receipt r  ON lt.`LedgerTransactionID`=r.`LedgerTransactionID`  AND r.Iscancel=0  ");                               
                    ...
                    sb.Append("  AND ROUND(lt.Adjustment)<ROUND(lt.netAmount) AND lt.iscredit=0 ");
