...
public partial class Design_OPD_SettlementReport : System.Web.UI.Page
{
    ...
    private bool reportaccess()
    {
        ...
        ReportAccessList response = JsonConvert.DeserializeObject<ReportAccessList>(objreport.ReportAccess(7));
    ...
    protected void btnPreview_Click(object sender, EventArgs e)
    {
        ...
        string startDate = string.Empty, toDate = string.Empty, Client, Users;
        ...
        Users = GetSelection(chklUser);

        GetCollectionData(startDate, toDate, Client, Users);
    }
    private void GetCollectionData(string fromDate, string toDate, string Client, string Users)
    {
        ...
        try
        {
            ...
            collections.Add("UserID", Users);
            collections.Add("ClientID", Client);
            collections.Add("CentreID", Centres);
            collections.Add("ReportType", rbtReportType.SelectedValue);
            collections.Add("PatientType", rblPatienttype.SelectedValue);
            collections.Add("SettlementType", rblsettlementtype.SelectedValue);
            collections.Add("ReportFormat", rdoReportFormat.SelectedValue);
            AllLoad_Data.POSTForm(collections, "Design/Reports/CollectionReport/SettlementReportPdf.aspx", this.Page);
        }
        catch (Exception ex)
        {
            ClassLog cl = new ClassLog();
