...
public partial class Design_OPD_CollectionReportNew : System.Web.UI.Page
{
    ...
    private bool reportaccess()
    {
        ...
        ReportAccessList response = JsonConvert.DeserializeObject<ReportAccessList>(objreport.ReportAccess(2));
        if (response.status == true)
        {
            if (response.responseMsg == "GlobalEmployee")
            {
                return true;
            }
            ...
            if (response.ShowPdf == 1 && response.ShowExcel == 0)
            {
                ...
                //ListItem li = rdoReportFormat.Items[0];
                //rdoReportFormat.Items.Remove(li);
            ...
            else if (response.ShowExcel == 1 && response.ShowPdf == 0)
            {
                ...
                //ListItem li = rdoReportFormat.Items[1];
                //rdoReportFormat.Items.Remove(li);
    ...
    private void GetCollectionData(string fromDate, string toDate, string userID)
    {
        ...
        string BaseCurrency = rblcurrencytype.SelectedValue;
        ...
        if (rdoReportFormat.SelectedValue == "")
        {
            lblMsg.Text = "Please Select Pdf or Excel format";
            return;
        }
        ...
        try
        {
            if (rbtReportType.SelectedValue == "0")
            {
                ...
                collections.Add("BaseCurrency", BaseCurrency);
                collections.Add("ReportFormat", rdoReportFormat.SelectedValue);
                AllLoad_Data.POSTForm(collections, "Design/Reports/CollectionReport/CollectionReportSummary.aspx", this.Page);
            ...
            else if (rbtReportType.SelectedValue == "1")
            {
                ...
                collections.Add("BaseCurrency", BaseCurrency);
                collections.Add("ReportFormat", rdoReportFormat.SelectedValue);
                AllLoad_Data.POSTForm(collections, "Design/Reports/CollectionReport/CollectionReport_Patientwisereport.aspx", this.Page);
