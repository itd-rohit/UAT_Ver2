...
public partial class Design_SalesDiagnostic_DailyStatus : System.Web.UI.Page
{
    ...
    public static string BindDailyStatus(string Pro, string Date)
    {
        ...
        sb.AppendLine(" SUM(pli.Amount) Achieved   FROM f_ledgertransaction lt    ");
        ...
        sb.AppendLine("  WHERE pli.Date>=concat('" + year + "-" + month + "-01 00:00:00') and pli.Date<=concat('" + year + "-" + month + "-31 23:59:59')  "); 
        if (Pro != "0")
        {
        ...
        sb.AppendLine(" group by lt.LedgertransactionID  )t ON t.ProID=tg.ProID WHERE DATE(t.Date)>= tg.TargetFromDate AND DATE(t.Date)<= tg.TargetToDate and month(tg.TargetToDate)=month('" + datetyu + "') ");
	...
	[WebMethod]
    public static string BindDailyStatusExcel(string Pro, string Date)
    {
        string _TeamMember = "";
        if (Pro != "0")
        {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            dt.Rows.Add(dr);
             ...
             HttpContext.Current.Session["ReportName"] = "Daily Status";
             return "1";
