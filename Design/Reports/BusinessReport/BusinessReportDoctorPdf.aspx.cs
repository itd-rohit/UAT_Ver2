...
public partial class Design_OPD_BusinessReportDoctorPdf : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            try
            {
                ...
                {
                    sb.Append(" SELECT lt.`Doctor_ID`,if(lt.Doctor_ID=2,'Other',lt.`DoctorName`)DoctorName,   ");
                    sb.Append("  SUM(Rate*Quantity)GrossAmount,SUM(Amount)NetAmount,SUM(DiscountAmt)DiscountAmt,sum((lt.NetAmount-lt.Adjustment))DueAmt ");//sum(lt.Adjustment)Adjustment,
                    ...
                    sb.Append("  GROUP BY lt.`Doctor_ID` order by DoctorName ");
                  
                }
                else if (Request.Form["ReportType"].ToString() == "1")
                {
                    sb = new StringBuilder();
                    sb.Append(" SELECT  DATE_FORMAT(plo.date, '%d-%b-%Y') DATE,plo.LedgerTransactionNo,plo.`LedgerTransactionID`,lt.`Doctor_ID`,lt.DoctorName,lt.`PName`,lt.Age, lt.Gender,lt.`PanelName`,cm.`Centre`, plo.`ItemName`,  ");
                    sb.Append(" ROUND((Rate * Quantity), 2) GrossAmount, ROUND((plo.Amount), 2) NetAmount,ROUND((DiscountAmt), 2) DiscountAmt, ROUND((lt.Adjustment), 2) Adjustment, ROUND((lt.NetAmount - lt.Adjustment)) DueAmt   ");
                    sb.Append(" FROM `patient_labinvestigation_opd` plo ");
                ...
                {
                    sb = new StringBuilder();
                    sb.Append(" SELECT lt.`Doctor_ID`,if(lt.Doctor_ID=2,'Other',lt.`DoctorName`)DoctorName,lt.`LedgerTransactionID`,lt.`LedgerTransactionNo`,lt.`PName`,lt.Age,lt.Gender,lt.`PanelName`,cm.`Centre`,   ");
                    sb.Append("  SUM(Rate*Quantity)GrossAmount,SUM(Amount)NetAmount,SUM(DiscountAmt)DiscountAmt,sum(lt.Adjustment)Adjustment,sum((lt.NetAmount-lt.Adjustment))DueAmt  ");
                    sb.Append(" FROM `patient_labinvestigation_opd` plo ");
                    sb.Append(" INNER JOIN f_ledgertransaction lt ON lt.`LedgerTransactionID`=plo.`LedgerTransactionID` INNER JOIN centre_master cm ON cm.`CentreID` = lt.CentreID ");
                    ...
                    sb.Append(" AND plo.date >=@fromDate AND plo.date <= @toDate  AND IF(plo.isPackage=1,plo.`SubCategoryID`=15,plo.`SubCategoryID`!=15) ");
                    sb.Append("  GROUP BY lt.`LedgerTransactionID`,lt.`Doctor_ID` order by DoctorName,lt.`LedgerTransactionID` ");
                    ...
                    using (MySqlDataAdapter da = new MySqlDataAdapter(String.Format(sb.ToString(), string.Join(",", UserDataList), string.Join(",", CentreIDDataList)), con))
                    {
                        ...
                        if (dt != null && dt.Rows.Count > 0)
                        {
                            ...
                            if (Request.Form["ReportFormat"].ToString() == "1")
                            {
                                ...
                                if (Request.Form["ReportType"].ToString() == "0")
                                {
                                    for (int i = 0; i < dt.Rows.Count; i++)
                                    {
                                        ...
                                        sb.AppendFormat(" <td style='width:20%;border-bottom: 1px solid;font-size: 14px !important;word-wrap: break-word;'>{0}</td> ", Util.GetString(dt.Rows[i]["DoctorName"]));
                                ...
                                {                                   
                                    ...
                                    for (int j = 0; j < distinctdoctor.Count; j++)
                                    {
                                        ...
                                        sb.Append(" <tr> ");
                                        sb.Append(" <td colspan='11' style='font-size: 16px;font-weight: bold;padding-top: 10px;padding-bottom: 10px;text-align:left;'>Doctor Name :  " + Util.GetString(dtdoc.Rows[0]["DoctorName"].ToString()) + "</td> ");
                                        sb.Append(" </tr> ");
                                        for (int i = 0; i < dtdoc.Rows.Count; i++)
                                        {
                                            if (Util.GetDecimal(dtdoc.Rows[i]["DueAmt"]) < 0)
                                                ...
                                                sb.Append("<tr> ");
                                            sb.AppendFormat(" <td style='width:2%;border-bottom: 1px solid;font-size: 14px !important;word-wrap: break-word;'>{0}</td> ", Util.GetInt(i + 1));
                                            sb.AppendFormat(" <td style='width:7%;border-bottom: 1px solid;font-size: 14px !important;word-wrap: break-word;'>{0}</td> ", Util.GetString(dtdoc.Rows[i]["DATE"]));
                                            sb.AppendFormat(" <td style='width:13%;border-bottom: 1px solid;font-size: 14px !important;word-wrap: break-word;'>{0}</td> ", Util.GetString(dtdoc.Rows[i]["PName"]));
                                            sb.AppendFormat(" <td style='width:6%;border-bottom: 1px solid;font-size: 14px !important;word-wrap: break-word;'>{0}</td> ", Util.GetString(dtdoc.Rows[i]["LedgerTransactionNo"]));
                                            ...
                                            sb.AppendFormat(" <td style='width:8%;border-bottom: 1px solid;font-size: 14px !important;word-wrap: break-word;'>{0}</td> ", Util.GetString(dtdoc.Rows[i]["PanelName"]));
                                            ...
                                            sb.AppendFormat(" <td style='width:10%;border-bottom: 1px solid;font-size: 14px !important;word-wrap: break-word;'>{0}</td> ", Util.GetString(dtdoc.Rows[i]["ItemName"]));
                                            ...
                                            sb.AppendFormat(" <td style='width:6%;border-bottom: 1px solid;font-size: 14px !important;word-wrap: break-word;'>{0}</td> ", Math.Round(Util.GetDecimal(dtdoc.Rows[i]["Adjustment"]), Util.GetInt(Resources.Resource.BaseCurrencyRound)), MidpointRounding.AwayFromZero);
                                        ...
                                        sb.Append("<tr> ");
                                        sb.AppendFormat(" <td colspan='9' style='width:60%;font-size: 18px;padding-top: 10px;font-weight: bold;text-align:right; '>SubTotal:</td>");
                                    ...
                                    sb.Append("<tr> ");
                                    sb.AppendFormat(" <td colspan='9' style='width:60%;font-size: 18px;padding-top: 10px;font-weight: bold;text-align:right; '>Grand Total:</td>");
                                ...
                                {                                   
                                    ...
                                    for (int j = 0; j < distinctdoctor.Count; j++)
                                    {
                                        DataTable dtdoc = dt.AsEnumerable().Where(x => x.Field<string>("DoctorName") == Util.GetString(distinctdoctor[j].DoctorName)).CopyToDataTable();
                                        sb.Append(" <tr> ");
                                        sb.Append(" <td colspan='10' style='font-size: 16px;font-weight: bold;padding-top: 10px;padding-bottom: 10px;text-align:left;'>Doctor Name : " + Util.GetString(dtdoc.Rows[0]["DoctorName"].ToString()) + "</td> ");
                                        sb.Append(" </tr> ");
                                        for (int i = 0; i < dtdoc.Rows.Count; i++)
                                        {
                                            if (Util.GetDecimal(dtdoc.Rows[i]["DueAmt"]) < 0)
                            ...
                            else
                            {
                                ...
                                Session["ReportName"] = "Doctor Wise Business Report";
    ...
    private string MakeHeader()
    {
        ...
        if (Request.Form["ReportType"].ToString() == "0")
        {
            sb.Append("<span style='font-weight: bold;font-size:20px;'>Doctor Wise Business Report(Summary)</span><br />");
        }
        else if (Request.Form["ReportType"].ToString() == "1")
        {
            sb.Append("<span style='font-weight: bold;font-size:20px;'>Doctor Wise Business Report(Detail)</span><br />");
        }
        else
        {
            sb.Append("<span style='font-weight: bold;font-size:20px;'>Doctor Wise Business Report(Detail)</span><br />");
        ...
        if (Request.Form["ReportType"].ToString() == "0")
        {
            ...
            sb.Append(" <td style='width:20%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>Doctor Name</td> ");
        ...
        {
            sb.Append(" <tr> ");
            sb.Append(" <td style='width:4%;font-weight: bold;font-size: 14px !important;word-wrap: break-word;'>Sr.No</td> ");
            sb.Append(" <td style='width:4%;font-weight: bold;font-size: 14px !important;word-wrap: break-word;'>Date</td> ");
            ...
            sb.Append(" <td style='width:6%;font-weight: bold;font-size: 14px !important;word-wrap: break-word;'>Age</td> ");
            sb.Append(" <td style='width:6%;font-weight: bold;font-size: 14px !important;word-wrap: break-word;'>Gender</td> ");
            sb.Append(" <td style='width:11%;font-weight: bold;font-size: 14px !important;word-wrap: break-word;'>Client Name</td> ");
            sb.Append(" <td style='width: 8%;font-weight: bold;font-size: 16px !important;word-wrap: break-word;'>Centre</td> ");
            sb.Append(" <td style='width:14%;font-weight: bold;font-size: 14px !important;word-wrap: break-word;'>Item Name</td> ");
            sb.Append(" <td style='width:8%;font-weight: bold;font-size: 14px !important;word-wrap: break-word;'>Gross Amt</td> ");
            ...
            sb.Append(" <td style='width:7%;font-weight: bold;font-size: 14px !important;word-wrap: break-word;'>Paid Amt</td> ");
