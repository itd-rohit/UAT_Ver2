...
public partial class Design_Reports_LabReport_MachineReadingPdf : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            try
            {
                ...
                sb.Append(" DATE_FORMAT(md.dtEntry,'%d-%b-%y') dtEntry,md.PName,'' Patient_ID,  ");
                sb.Append(" md.Age,md.Gender,md.MachineName FROM mac_data md WHERE md.MachineName<>'' and md.reading<>''  ");


                if (Request.Form["Machine"].ToString() != string.Empty)
                {
                    sb.Append(" and md.MachineName= @Machine ");
                }
                sb.Append(" and md.MacDate>= @fromDate and md.MacDate<= @toDate ");
                ...
                try
                {
                    ...
                    {                       
                        ...
                        da.SelectCommand.Parameters.AddWithValue("@Machine", Request.Form["Machine"].ToString());
                        da.Fill(dt);

                       
                        if (dt != null && dt.Rows.Count > 0)
                        {
                            Period = string.Concat(" Period From :", Util.GetDateTime(Request.Form["fromDate"].ToString()), " Period To :", Util.GetDateTime(Request.Form["toDate"].ToString()));
                            if (Request.Form["ReportFormat"].ToString() == "1")
                            {
                                ...
                                sb.Append(" <td style='width: 10%;font-weight: bold;font-size: 14px;border-top: 2px solid; border-bottom: 2px solid;'>BarcodeNo</td> ");
                                sb.Append(" <td style='width: 10%;font-weight: bold;font-size: 14px;border-top: 2px solid; border-bottom: 2px solid;'>PatientName</td> ");
                                sb.Append(" <td style='width: 10%;font-weight: bold;font-size: 14px;border-top: 2px solid; border-bottom: 2px solid;'>TestID</td> ");
                                sb.Append(" <td style='width: 10%;font-weight: bold;font-size: 14px;border-top: 2px solid; border-bottom: 2px solid;'>Reading</td> ");

                                sb.Append(" <td style='width: 10%;font-weight: bold;font-size: 14px;border-top: 2px solid; border-bottom: 2px solid;'>Machine Name</td> ");
                                ...
                                {
                                    sb.Append("<tr> ");
                                    sb.Append(" <td style='width: 10%;font-size: 14px;border-top: 1px solid; border-bottom: 1px solid;'>" + Util.GetString(dt.Rows[i]["dtEntry"]) + "</td> ");
                                    ...
                                    sb.Append(" <td style='width: 10%;font-size: 14px;border-top: 1px solid; border-bottom: 1px solid;'>" + Util.GetString(dt.Rows[i]["PName"]) + "</td> ");
                                    sb.Append(" <td style='width: 10%;font-size: 14px;border-top: 1px solid; border-bottom: 1px solid;'>" + Util.GetString(dt.Rows[i]["LabObservation_ID"]) + "</td> ");
                                    sb.Append(" <td style='width: 10%;font-size: 14px;border-top: 1px solid; border-bottom: 1px solid;'>" + Util.GetString(dt.Rows[i]["Reading"]) + "</td> ");
                                    sb.Append(" <td style='width: 10%;font-size: 14px;border-top: 1px solid; border-bottom: 1px solid;'>" + Util.GetString(dt.Rows[i]["MachineName"]) + "</td> ");
                            ...
                            else
                            {
                                ...
                                Session["ReportName"] = "Machine Reading Report";
    ...
    private string MakeHeader()
    {
        ...
        sb.Append("<td style='width:100%; text-align: center;'>");
            ...
            sb.Append("<span style='font-weight: bold;font-size:20px;'>Machine Reading Report </span><br />");
