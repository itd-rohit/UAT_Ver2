...
public partial class Design_Reports_WorkSheet_DepartmentWisePendingListPdf : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            try
            {
                ...
                try
                {
                    using (DataTable dt = ((DataTable)Session["dtDeptPendingList"]))
                    {                        

                        if (dt != null && dt.Rows.Count > 0)
                        {
                            Period = string.Concat(" Period From :", Util.GetString(dt.Rows[0]["ReportHeading"]));
                            ...
                            sb.Append(" <td style='width: 30%;font-weight: bold;font-size: 14px;border-top: 2px solid; border-bottom: 2px solid;'>TestName</td> ");
                            sb.Append(" <td style='width: 15%;font-weight: bold;font-size: 14px;border-top: 2px solid; border-bottom: 2px solid;'>SRA Date Time</td> ");

                            sb.Append(" <td style='width: 10%;font-weight: bold;font-size: 14px;border-top: 2px solid; border-bottom: 2px solid;'>Status</td> ");
                            sb.Append(" <td style='width: 10%;font-weight: bold;font-size: 14px;border-top: 2px solid; border-bottom: 2px solid;'>IsRerun</td> ");
                            ...
                            sb.Append(" </tr> ");
                                ...
                                for (int i = 0; i < dt.Rows.Count; i++)
                                {
                                    ...
                                    sb.Append(" <td style='width: 30%;font-size: 14px;border-top: 1px solid; border-bottom: 1px solid;'>" + Util.GetString(dt.Rows[i]["TestName"]) + "</td> ");
                                    sb.Append(" <td style='width: 15%;font-size: 14px;border-top: 1px solid; border-bottom: 1px solid;'>" + Util.GetString(dt.Rows[i]["LogisticReceiveDate"]) + "</td> ");

                                    sb.Append(" <td style='width: 10%;font-size: 14px;border-top: 1px solid; border-bottom: 1px solid;'>" + Util.GetString(dt.Rows[i]["CurrentStatus"]) + "</td> ");
                                    sb.Append(" <td style='width: 10%;font-size: 14px;border-top: 1px solid; border-bottom: 1px solid;'>" + Util.GetString(dt.Rows[i]["IsRerun"]) + "</td> ");                                    
                                    sb.Append(" </tr> ");
                                }
            ...
            finally
            {
                ...
                Session["dtDeptPendingList"] = "";
                Session.Remove("dtDeptPendingList");
            }
        }
    }
    ...
    private string MakeHeader()
    {
        ...
        sb.Append("<span style='font-weight: bold;font-size:20px;'>Technician Pending List  Report</span><br />");
