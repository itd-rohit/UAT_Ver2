...
public partial class Design_OPD_Default : System.Web.UI.Page
{
    ...
    void binddept()
    {
        ddldepartment.DataSource = StockReports.GetDataTable("select SubCategoryID,name from f_subcategorymaster where SubCategoryID<>'LSHHI24' and active=1 AND `SubcategoryID`<>15  order by name");
    ...
    public DataTable getreport()
    {
        ...
        {           
            ...
            sb.Append(" concat(plo.ItemCode,'-', UPPER(plo.ItemName)) TestName, ");
            if (ddlDateType.SelectedValue == "lt.Date")
            {
              sb.Append(" day(lt.Date) Day,count(plo.investigation_id) CountMe,plo.investigation_id,plo.`SubCategoryID`  ");
	    }
            else if (ddlDateType.SelectedValue == "plo.Date")
            {
              sb.Append(" day(plo.Date) Day,count(plo.investigation_id) CountMe,plo.investigation_id,plo.`SubCategoryID`  ");
	    }
            else if (ddlDateType.SelectedValue == "plo.ResultEnteredDate")
            {
              sb.Append(" day(plo.ResultEnteredDate) Day,count(plo.investigation_id) CountMe,plo.investigation_id,plo.`SubCategoryID`  ");
	    }
            else if (ddlDateType.SelectedValue == "plo.ApprovedDate")
            {
            ...
            sb.Append(" where plo.IsActive = 1  AND plo.IsReporting = '1' AND plo.isRefund=0  ");//AND plo.IsSampleCollected='Y'
            if (ddlDateType.SelectedValue == "lt.Date")
            {
            ...
            {
                sb.Append(" AND plo.ResultEnteredDate >=@fromdate  AND  plo.ResultEnteredDate <=@todate  ");
            }
            else if (ddlDateType.SelectedValue == "plo.ApprovedDate")
            {
                sb.Append(" AND plo.ApprovedDate >=@fromdate  AND  plo.ApprovedDate <=@todate and plo.Approved=1  ");
            }
          ...
          //  sb.Append(" AND IF(plo.isPackage = 1,plo.`SubCategoryID` = 15,plo.`SubCategoryID` != 15)  ");
            if (ddlDateType.SelectedValue == "lt.Date")
            {
               sb.Append(" GROUP BY plo.investigation_id,day(lt.Date) ");
               sb.Append(" ORDER BY day(lt.Date),TestName");
            }
           else if (ddlDateType.SelectedValue == "plo.Date")
            {
               sb.Append(" GROUP BY plo.investigation_id,day(plo.Date) ");
               sb.Append(" ORDER BY day(plo.Date),TestName");
            }
            else if (ddlDateType.SelectedValue == "plo.ResultEnteredDate")
            {
               sb.Append(" GROUP BY plo.investigation_id,day(plo.ResultEnteredDate) ");
               sb.Append(" ORDER BY day(plo.ResultEnteredDate),TestName");
            }
             else if (ddlDateType.SelectedValue == "plo.ApprovedDate")
            {
         ...
         //   System.IO.File.WriteAllText(@"C:\ITDOSE\LiveCode\Oncquest\ErrorLog\Report123213.txt", sb.ToString());
