...
public partial class Design_Mobile_Login : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //string UserID = Util.GetString(Request.Form["UserID"]);
        //string Password = Util.GetString(Request.Form["Password"]);
        string UserID = Util.GetString(Request.QueryString["UserID"]);
        string Password = Util.GetString(Request.QueryString["Password"]);
        if (UserID != "" && Password != "")
        {
    ...
    private void UserLogin()
    {
        try
        {
            ...
            if (InvalidPassword >= 3)
            {
                ...
                Response.ClearHeaders();
                Response.AddHeader("Content-Type", "text/plain");
                Response.Write("You have exceeded the number of allowed login attempts");
                Response.Flush();
                Response.End();  
                return;
            }
