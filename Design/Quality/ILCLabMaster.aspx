<%@ Page Title="" Language="C#" MasterPageFile="~/Design/DefaultHome.master" AutoEventWireup="true" CodeFile="ILCLabMaster.aspx.cs" Inherits="Design_Quality_ILCLabMaster" %>
 ...
 <Ajax:ScriptManager ID="ScriptManager1" runat="server" EnablePageMethods="true">
     ...
     <div id="Pbody_box_inventory" style="width:1304px;">
            ...
            <div class="content">
                <table width="99%">
                    <tr>
                         <td style="font-weight: 700; color: #FF0000;">ILC Lab Name :</td>
                         <td><asp:TextBox ID="txtilclabname" runat="server" Width="400px" />
                             <asp:TextBox ID="txtilclabid" runat="server" style="display:none;" />

                         </td>
                         <td style="font-weight: 700">ILC Lab Address :</td>
                         <td><asp:TextBox ID="txtilclabaddress" runat="server" Width="500px" /></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                         <td style="font-weight: 700;color: #FF0000;">MOU Renew Date :</td>
                        ...
                        <cc1:CalendarExtender ID="FromdateCal" TargetControlID="txtrenewdate" PopupButtonID="txtrenewdate" Format="dd-MMM-yyyy" runat="server">
                         ...
                         <td style="font-weight: 700; color: #FF0000;">MOU Document :
                             </td>
                         <td> 
                ...
                <div  style="width:1295px; max-height:400px;overflow:auto;">
                   <table id="tblitemlist" style="width:99%;border-collapse:collapse;text-align:left;">
                                     <tr id="triteheader">
                                        ...
                                        <td class="GridViewHeaderStyle">ILC Lab Name</td>
                                        <td class="GridViewHeaderStyle">ILC Lab Address</td>
                                        <td class="GridViewHeaderStyle">Contact No</td>
                                        <td class="GridViewHeaderStyle">Contact Person Name</td>
                                        <td class="GridViewHeaderStyle">MOU Renew Date</td>
                                        <td class="GridViewHeaderStyle">MOU Document</td>
    ...
    <script type="text/javascript">
        ...
        function reset() {
            ...
            $('#<%=txtilclabname.ClientID%>').val('');
            $('#<%=txtilclabaddress.ClientID%>').val('');
            ...
            $('#<%=txtilclabid.ClientID%>').val('');
        }

        
        function savelab() {

            if ($('#<%=txtilclabname.ClientID%>').val() == "") {
                $('#<%=txtilclabname.ClientID%>').focus();
                showerrormsg("Please Enter ILC Lab Name");
                return;
            }

            if ($('#<%=txtrenewdate.ClientID%>').val() == "") {
                $('#<%=txtrenewdate.ClientID%>').focus();
                 showerrormsg("Please Select MOU Renew Date");
                 return;
            }
            if ($('#<%=txtfilename.ClientID%>').val() == "") {
              
                  showerrormsg("Please Upload MOU Document");
                  return;
            }
            var IsActive = "0";
            ...
            if (IsActive == "0") {
                ...
                showerrormsg("Please Check Active CheckBox For First Time ILC Lab Creation");
            ...
            //$.blockUI();
            $.ajax({
                url: "ILCLabMaster.aspx/savedata",
                data: '{ labname:"' + $('#<%=txtilclabname.ClientID%>').val() + '",labaddress:"' + $('#<%=txtilclabaddress.ClientID%>').val() + '",contactno:"' + $('#<%=txtcontactno.ClientID%>').val() + '",contactpername:"' + $('#<%=txtcontactname.ClientID%>').val() + '",renewdate:"' + $('#<%=txtrenewdate.ClientID%>').val() + '",IsActive:"' + IsActive + '"}', // parameter map 
        ...
        function updatelab() {
            ...
            //$.blockUI();
            $.ajax({
                url: "ILCLabMaster.aspx/updatedata",
                data: '{labid:"' + $('#<%=txtilclabid.ClientID%>').val() + '", labname:"' + $('#<%=txtilclabname.ClientID%>').val() + '",labaddress:"' + $('#<%=txtilclabaddress.ClientID%>').val() + '",contactno:"' + $('#<%=txtcontactno.ClientID%>').val() + '",contactpername:"' + $('#<%=txtcontactname.ClientID%>').val() + '",renewdate:"' + $('#<%=txtrenewdate.ClientID%>').val() + '",IsActive:"' + IsActive + '"}', // parameter map 
                type: "POST", // data has to be Posted    	        
                contentType: "application/json; charset=utf-8",
                timeout: 120000,
                dataType: "json",
        ...
        function binddata() {
            ...
            $.ajax({
                url: "ILCLabMaster.aspx/binddata",
                data: '{}',
                type: "POST",
                timeout: 120000,
                ...
                success: function (result) {
                    ...
                    if (ItemData.length == 0) {
                        ...
                        for (var i = 0; i <= ItemData.length - 1; i++) {
                            var mydata = "<tr style='background-color:lightgreen;' id='" + ItemData[i].LabID + "'>";
                            mydata += '<td class="GridViewLabItemStyle"  id="srno">' + parseInt(i+1) + '</td>';
                            mydata += '<td class="GridViewLabItemStyle"  id="ILCLabName">' + ItemData[i].ILCLabName + '</td>';
                            mydata += '<td class="GridViewLabItemStyle"  id="ILCLabAddress">' + ItemData[i].ILCLabAddress + '</td>';
                            ...
                            mydata += '<td class="GridViewLabItemStyle"  id="MOURenewDate">' + ItemData[i].MOURenewDate + '</td>';
                            mydata += '<td class="GridViewLabItemStyle"  id="MOUDocument"><a style="font-weight:bold;" target="_blank" href="MOUDocument/' + ItemData[i].MOUDocument + '">' + ItemData[i].MOUDocument + '</a></td>';
        ...
        function showdetailtoupdate(ctrl) {
            ...
            $('#<%=txtilclabid.ClientID%>').val($(ctrl).closest("tr").attr('id'));
            $('#<%=txtilclabname.ClientID%>').val($(ctrl).closest("tr").find('#ILCLabName').html());
            $('#<%=txtilclabaddress.ClientID%>').val($(ctrl).closest("tr").find('#ILCLabAddress').html());
            ...
            $('#<%=txtrenewdate.ClientID%>').val($(ctrl).closest("tr").find('#MOURenewDate').html());
