...
public partial class Design_Quality_ILCResultReject : System.Web.UI.Page
{
    ...
    [WebMethod]
    public static string getilcresult(string processingcentre, string regisyearandmonth)
    {
        StringBuilder sb = new StringBuilder();
        sb.Append(" SELECT isReject, cm.centre, qil.id, test_id,`LedgerTransactionNO`,barcodeno,InvestigationID,im.name TestName,LabobservationId,IFNULL(lom.name,im.name) ParameterName, ");
        sb.Append(" IFNULL(qilm1.ilclabname,qilm.ilclabname)ilclabname, ");
        ...
        sb.Append("LEFT JOIN labobservation_master lom ON lom.`LabObservation_ID`=LabobservationId ");
        sb.Append("LEFT JOIN qc_ilcparametermapping qilm ON qilm.`ProcessingLabID`=qil.`CentreID` AND qilm.`TestType`='Observation' ");
        sb.Append("AND  qilm.`TestID`=qil.`LabObservationID` and qilm.IsActive=1 ");

        sb.Append("LEFT JOIN qc_ilcparametermapping qilm1 ON qilm1.`ProcessingLabID`=qil.`CentreID` AND qilm1.`TestType`='Investigation' ");
        sb.Append("AND  qilm1.`TestID`=qil.`InvestigationID`  and qilm1.IsActive=1 ");
    ...
    public static string rejectnow(List<string> ILCResultData, string rejectreason)
    {
        ...
        try
        {
            ...
            foreach (string ilc in ILCResultData)
            {
                ...
                MySqlHelper.ExecuteNonQuery(tnx, CommandType.Text, "update qc_ilcregistration set isReject=@isReject,RejectDateTime=@RejectDateTime,RejectByName=@RejectByName,RejectByID=@RejectByID,RejectReason=@RejectReason where id=@id",
