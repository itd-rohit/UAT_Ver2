...
public partial class Design_B2CMobile_MobileAppB2CSetting : System.Web.UI.Page
{
    ...
    public static string BindData()
    {
        ...
        sb.Append("  SELECT ID,Color,ifnull(WelcomeContent,'')WelcomeContent ,ifnull(WelcomeText,'')WelcomeText,Concat('../B2CMobile/Images/','',IF(Logo='','default.jpg',Logo))Logo,IF(IsShowPoweredBy=0,'No','Yes')IsShowPoweredBy,HeaderTest,IFNULL(LabReportPath,'')LabReportPath,HelpLineNo24x7 from App_B2C_Setting  Limit 1 ");
            ...
            if (data.Count > 0)
            {
                for (int i = 0; i < data.Count; i++)
                {
                    ...
                    if (data[i].LoginLogo != "")
                    {
                        ...
                        byte[] byteBuffer = Convert.FromBase64String(data[i].LoginLogo.Split(',')[1].ToString());
                        using (MemoryStream ms = new MemoryStream(byteBuffer))
                        {
                            using (Bitmap bm2 = new Bitmap(ms))
                            {
                                bm2.Save(HttpContext.Current.Server.MapPath("~/Design/B2CMobile/Images/" + data[i].FileName));
                            }
                        }
                    }

                    StringBuilder sb = new StringBuilder();
                    string ab = (StockReports.ExecuteScalar("SELECT count(*) FROM App_B2C_Setting  Limit 1;"));
                    if (ab != "0")
                    {
                        sb.Append(" Update App_B2C_Setting  SET HeaderTest=@HeaderTest,WelcomeContent=@WelcomeContent,Color=@Color,WelcomeText=@WelcomeText,UpdateById=@EntryById,UpdateBy=@EntryBy,IsShowPoweredBy=@IsShowPoweredBy,LabReportPath=@LabReportPath,HelpLineNo24x7=@HelpLineNo24x7");
                        if (data[i].LoginLogo != "")
                            sb.Append(" ,Logo=@Logo ");
                        ...
                        MySqlHelper.ExecuteNonQuery(tranX, CommandType.Text, sb.ToString(),
                               ...
                               new MySqlParameter("@ID", data[i].Appid)
                               );
                    ...
                    else
                    {
                        ...
                        sb.Append("INSERT INTO `App_B2C_Setting`(HeaderTest,Color,Logo,WelcomeText,EntryById,EntryBy,EntryDate,IsShowPoweredBy,LabReportPath,WelcomeContent,HelpLineNo24x7)");
    ...
    public class Datalist
    {
        ...
        private string _LoginContent2;
        private string _ThemeColor;
        private string _LoginLogo;
        private string _FileName;
        private string _LoginContent1;
        private string _LabReportPath;
        private string _WelcomeContent;
        private string _Helpline24x7;
        public string WelcomeContent
        {
            get { return _WelcomeContent; }
            set { _WelcomeContent = value; }
        }
        ...
        {
            get { return _LoginContent2; }
            set { _LoginContent2 = value; }
        }
        ...
        {
            get { return _LoginContent1; }
            set { _LoginContent1 = value; }
        }
        ...
        {
            get { return _LabReportPath; }
            set { _LabReportPath = value; }
        }
        public string Helpline24x7
        {
            get { return _Helpline24x7; }
            set { _Helpline24x7 = value; }
        }
