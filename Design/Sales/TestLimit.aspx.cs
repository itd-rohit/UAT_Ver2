...
public partial class Design_Sales_TestLimit : System.Web.UI.Page
{
    ...
    public static string bindSalesHierarchy(string EntryType)
    {
        ...
        sb.Append("SELECT sh.ID SalesID,sh.Name SalesName,IFNULL(TestLimit,'')TestLimit,IFNULL(BusinessCommitment,'0')BusinessCommitment,IFNULL(sl.ID,1)ID,'' `Add`,'' `Remove` FROM f_designation_msater  sh ");
        sb.Append(" LEFT JOIN sales_special_testlimit_count sl ON sh.ID=sl.SalesID AND sl.IsActive=1 AND sl.EntryType='" + EntryType.ToUpper() + "'");
        sb.Append(" ");
                ...
                if (RowCreated.Length == 0)
                {
                    ...
                    if (RowExist.Length > 0)
                    {
                        ...
                        for (int i = 0; i < RowExist.Length; i++)
                        {
                            ...
                            row[RowExist[i]["SalesName"].ToString() + "#" + RowExist[i]["SalesID"].ToString()] = RowExist[i]["TestLimit"].ToString();
    ...
    public static string saveTestLimit(string ItemDetail, string EntryType)
    {
        ...
        try
        {
            ...
            ItemDetails = ItemDetails.Where(s => !string.IsNullOrWhiteSpace(Util.GetString(s.testLimit))).ToList();

            MySqlHelper.ExecuteNonQuery(tnx, CommandType.Text, "UPDATE sales_special_testlimit_count SET IsActive=@IsActive WHERE EntryType=@EntryType",
            ...
            for (int k = 0; k < ItemDetails.Count; k++)
            {
                ...
                sb.Append(" INSERT INTO sales_special_testlimit_count(BusinessCommitment,SalesID,TestLimit,CreatedByID,CreatedBy,EntryType)");
                sb.Append(" VALUES(@BusinessCommitment,@SalesID,@TestLimit,@CreatedByID,@CreatedBy,@EntryType)");

                MySqlHelper.ExecuteNonQuery(tnx, CommandType.Text, sb.ToString(),
                     new MySqlParameter("@BusinessCommitment", ItemDetails[k].businessCommitment),
                     new MySqlParameter("@SalesID", ItemDetails[k].salesID), new MySqlParameter("@TestLimit", ItemDetails[k].testLimit),
