...
public partial class Design_Sales_SalesVerification : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            lblIsSales.Text = Common.Encrypt("Sales");
        }
    ...
    [WebMethod]
    public static string bindSalesEnrolment(string fromDate, string toDate)
    {
        ...
        sb.Append(" IsSalesApproved ShowButton");
    ...
    [WebMethod]
    public static string checkSalesEnrolment(string ID)
    {
        int count = Util.GetInt(StockReports.ExecuteScalar(string.Format("SELECT COUNT(*) FROM sales_enrolment_master WHERE ID='{0}' AND IsSalesApproved=1", ID)));
        if (count > 0)
            return "1";
        else
