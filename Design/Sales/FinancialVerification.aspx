<%@ Page  Language="C#" MasterPageFile="~/Design/DefaultHome.master" ClientIDMode="Static" AutoEventWireup="true" CodeFile="FinancialVerification.aspx.cs" Inherits="Design_Sales_FinancialVerification" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
     ...
     <div id="Pbody_box_inventory" style="width:1304px;">
        ...
        <div class="POuter_Box_Inventory" style="text-align: center;width:1300px;">
            <b>Financial Admin Verification Search</b><br />
            <asp:Label ID="lblMsg" runat="server" Font-Bold="True" ForeColor="Red" ClientIDMode="Static"></asp:Label>
             <asp:Label ID="lblIsFinancial" runat="server" style="display:none" ClientIDMode="Static"></asp:Label>
        </div>
...
<td style="text-align: center; font-weight: 700;" colspan="4">
    <input type="button" value="Search" class="searchbutton" onclick="searchVerification()" id="btnSearch" />
     ...
     <script type="text/javascript">
         function onSucessFinancialSearch(result) {
             FinancialData = jQuery.parseJSON(result);
             if (FinancialData != null) {
                 var output = jQuery('#sc_FinancialVerified').parseTemplate(FinancialData);
                 jQuery('#divEnrolment').html(output);
                 jQuery('#<%=lbltotal.ClientID%>').text(FinancialData.length);
                }
                else {
         ...
         function searchVerification() {
             ...
             jQuery('#lblMsg').text('');
             PageMethods.bindFinancialEnrolment(jQuery('#<%=txtFromDate.ClientID%>').val(), jQuery('#<%=txtToDate.ClientID%>').val(), onSucessFinancialSearch, onFailureEnroll);
         ...
         function onSucessEnroll(result) {
             if (result == "1")
                 ...
                 location.href = 'EnrolmentVerification.aspx?EnrolID=' + result + '&Verified=' + jQuery("#lblIsFinancial").text() + '';
         }
         function onFailureEnroll(result) {

         }
         function ShowDetail(rowID) {
             PageMethods.checkFinancialEnrolment($(rowID).closest('tr').find('#tdID').text(), onSucessEnroll, onFailureEnroll);
         }
             </script>
     <script id="sc_FinancialVerified" type="text/html">
        <table class="GridViewStyle" cellspacing="0" rules="all" border="1" id="tb_FinancialVerified"   style="border-collapse:collapse; width:100%;">
		<tr id="Header">
...
<#  var dataLength=FinancialData.length;
             
              var objRow;   
        for(var j=0;j<dataLength;j++)
        {

        objRow = FinancialData[j];
