<%@ Page  Language="C#" MasterPageFile="~/Design/DefaultHome.master" ClientIDMode="Static" AutoEventWireup="true" CodeFile="TestLimit.aspx.cs" Inherits="Design_Sales_TestLimit" %>
  ...
  <div id="Pbody_box_inventory" style="width: 1304px;">
      <Ajax:ScriptManager ID="ScriptManager1" runat="server" EnablePageMethods="true">
        ...
        <div class="POuter_Box_Inventory" style="text-align: center;width: 1300px;">           
                <b>Special Test Count Limit Master</b>
              <br />
    ...
    <script type="text/javascript">
                ...
                jQuery.each(this, function (k, v) {
                    if (k == 'ID') {
                        ...
                        if (k == 'BusinessCommitment')
                            tblrow.push("<td class='GridViewLabItemStyle " + k.substr(k.indexOf('#') + 1) + "'><input type='text' style='width:90px' id='" + k.substr(k.indexOf('#') + 1) + "' class='BusinessCommitment' value='" + testLimit[0] + "'/></td>");
                        else
                            tblrow.push("<td class='GridViewLabItemStyle " + k.substr(k.indexOf('#') + 1) + "'><input type='text' style='width:70px' id='" + k.substr(k.indexOf('#') + 1) + "' class='testValue' value='" + testLimit[0] + "'/></td>");
                    }
                    if (k == 'BusinessCommitment') {
                        count = count + 1;
                    }
        ...
        function addDetail(rowID) {
            ...
            jQuery(rowID).closest('tr').find(".testValue").each(function (event) {
                var testLimit = jQuery.trim(jQuery(this).closest('td').find(".testValue").val());
                if (testLimit == "") {
                ...
                var testLimit1 = jQuery(this).closest('td').next('td').find(".testValue").val();

                if (testLimit1 == "" && testLimit1 != 'undefined') {
                    showerrormsg('Please Enter Test Limit');
                ...
                if (parseInt(testLimit) > parseInt(testLimit1)) {
        ...
        function saveTestLimit() {
            ...
            jQuery("#mtable tbody").find('tr').each(function (key, val) {
                ...
                jQuery(".testValue", this).each(function () {
                    ...
                    var testLimit = this.value;
                    if (testLimit.length != 0 && testLimit.length != '' && testLimit != 'null') {
                        item.businessCommitment = businessCommitment;
                        item.salesID = SalesID;
                        item.testLimit = testLimit;
                        dataItem.push(item);
                        item = new Object();
                    }
                });
            ...
            if (dataItem.length > 0) {
                jQuery.ajax({
                    type: "POST",
                    url: "TestLimit.aspx/saveTestLimit",
