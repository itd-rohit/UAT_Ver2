            ...
            if (data.Count > 0)
            {
                for (int i = 0; i < data.Count; i++)
                {
                    ...
                    if (data[i].LoginLogo != "")
                    {
                        ...
                        byte[] byteBuffer = Convert.FromBase64String(data[i].LoginLogo.Split(',')[1].ToString());App_B2B_Setting
                    ...
                    StringBuilder sb = new StringBuilder();
                    string ab = (StockReports.ExecuteScalar("SELECT count(*) FROM App_B2B_Setting  Limit 1;"));
                    if (ab != "0")
                    {
