...
public partial class Design_Store_DirectPO : System.Web.UI.Page
{
	...
	public static string SearchItem(string itemname, string SupplierID, string itemcate)
    {
        StringBuilder sb = new StringBuilder();

        sb.Append(" SELECT si.`ItemID`,sig.`ItemNameGroup`ItemName ,si.`ItemIDGroup` ");
        sb.Append(" FROM st_itemmaster si     ");
        sb.Append(" INNER JOIN `st_itemmaster_group` sig ON sig.`ItemIDGroup`=si.`ItemIDGroup` ");
        sb.Append(" INNER JOIN `st_vendorqutation` ven ON ven.`ItemID`=si.`ItemID` AND ven.isactive=1 AND ven.approvalstatus=2 AND ven.ComparisonStatus=1 and EntryDateTo>=date(now()) ");
        //  sb.Append(" AND ven.`VendorId` ='" + SupplierID + "'  AND DeliveryLocationID='" + DeliveryLocationID + "' ");
        sb.Append(" AND ven.`VendorId` ='" + SupplierID + "'  ");
        sb.Append(" and si.categorytypeid='" + itemcate + "' ");
        sb.Append(" AND typeName LIKE '" + itemname + "%'  ");
        sb.Append(" ORDER BY sig.`ItemNameGroup` LIMIT 20   ");
		//System.IO.File.WriteAllText(@"C:\itdose\Livecode\Droplet_Live_New\Droplet\Design\Store\directpo.txt",sb.ToString());
        return Newtonsoft.Json.JsonConvert.SerializeObject(StockReports.GetDataTable(sb.ToString()));
    }
