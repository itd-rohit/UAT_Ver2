...
public partial class Design_HomeCollection_HomecollectionSearch : System.Web.UI.Page
{
    ...
    public static string bindroute()
    {
        ...
        {
            StringBuilder sb = new StringBuilder();
            sb.Append(" SELECT RouteID,Route FROM " + Util.getApp("HomeCollectionDB") + ".hc_routemaster WHERE isActive=1 ORDER BY Route ");
    ...
    [WebMethod(EnableSession = true)]
    public static string getdata(string fromdate, string todate, string stateId, string cityid, string areaid, string pincode, string centre, string phelbo, string mobileno, string pname, string status, string dateoption, string prebookingno,string route,string fromtime,string totime)
    {
        MySqlConnection con = Util.GetMySqlCon();
        con.Open();
        try
        {
            ...
            sb.Append(" concat(plo.title,plo.pname)patientname,plo.age,plo.gender,plo.locality,plo.city,plo.state,plo.pincode,if(cm.type1='PCC',concat(cm.centre,'~',cm.COCO_FOCO),cm.centre) as centre, ");
            ...
            sb.Append(" (Select LedgertransactionNo FROM `f_ledgertransaction` WHERE PreBookingID=plo.PreBookingID limit 1)LedgertransactionNo ");
            ...
            if (route != "0" && route != null && route != "")
            {
                sb.Append(" and hc.route_ID=@route ");
            }
            if (pname != "")
            {
            ...
            sb.Append(" GROUP BY plo.prebookingid ORDER BY hc.appdatetime,prebookingid ");
// return sb.ToString();
            using (DataTable dt = MySqlHelper.ExecuteDataset(con, CommandType.Text, sb.ToString(),
                                             ...
                                             new MySqlParameter("@fromdate", string.Concat(Util.GetDateTime(fromdate).ToString("yyyy-MM-dd"), " "+ fromtime)),
                                             new MySqlParameter("@todate", string.Concat(Util.GetDateTime(todate).ToString("yyyy-MM-dd"), " " + totime)),
